...
function LoginPage() {
  ...
  const [companyName, setCompanyName] = useState(
    location.state?.prefillCompany || import.meta.env.VITE_COMPANY_NAME || "",
  ); // Pre-filled from navigation state or env
  ...
  const handleSubmit = async (e) => {
    ...
    if (!companyName) {
      console.error(
        "Error: VITE_API_LOGIN_BASE_URL or Company Name is missing.",
      );
      setMessage(
        "خطأ: يرجى إدخال اسم الشركة وعنوان URL الأساسي لتسجيل الدخول.",
      );
      setLoading(false);
      return;
    }
