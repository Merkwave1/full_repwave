      ...
      setFormData({
        ...
        variants_data: (product.variants || []).map((v) => ({
          ...
          attribute_value_ids: v.attributes
            ? v.attributes.map((attr) => attr.attribute_value_id)
            : [],
        })),
  ...
  const handleChange = (e) => {
    ...
    if (name === "products_image" && files && files[0]) {
      ...
      setFormData((prevData) => ({
        ...prevData,
        products_image: file,
        products_image_url: URL.createObjectURL(file),
      }));
  ...
  const handleVariantChange = (index, e) => {
    ...
    if (name === "variant_image" && files && files[0]) {
      ...
      newVariants[index] = {
        ...newVariants[index],
        variant_image: file,
        variant_image_url: URL.createObjectURL(file),
      };
  ...
  const handleSubmit = (e) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (
        key !== "products_image" &&
        key !== "variants_data" &&
        key !== "preferred_packaging_ids" &&
        key !== "products_image_url"
      ) {
    ...
    apiFormData.append(
      "preferred_packaging_ids",
      JSON.stringify(formData.preferred_packaging_ids),
    );
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end space-y-reverse space-y-3 sm:space-y-0 sm:space-x-4 sm:space-x-reverse mt-6 sm:mt-8 pt-4 sm:pt-5 border-t border-gray-200">
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-6 py-2.5 sm:py-2 border border-transparent rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 text-base sm:text-sm"
          >
            تحديث المنتج
          </button>
        </div>
