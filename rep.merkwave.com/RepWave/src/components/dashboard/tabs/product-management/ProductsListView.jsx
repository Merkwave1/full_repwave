  ...
  const columns = [
    {
      ...
      render: (p) => (
        <span className="bg-blue-50 text-[#1F2937] text-xs px-2 py-1 rounded-full font-semibold">
          {p.products_id}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (p) => getCategoryName(p.products_category_id),
    },
    {
      key: "products_supplier_id",
      title: "المورد",
      sortable: true,
      headerClassName: "min-w-[90px]",
      render: (p) => getSupplierName(p.products_supplier_id),
    },
    {
      key: "variants",
      title: "الخيارات",
      ...
      render: (p) =>
        Array.isArray(p.variants) && p.variants.length > 0 ? (
          p.variants.map((v, idx) => (
            <div
              key={idx}
              className="text-xs text-gray-700 flex items-center gap-2 flex-wrap"
            >
              <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded text-[10px]">
                #{v.variant_id}
              </span>{" "}
              {v.variant_name || `خيار ${idx + 1}`} (
              {v.variant_sale_price ?? v.variant_unit_price}){" "}
              {odooEnabled && v.variant_odoo_product_id && (
                <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded text-[10px]">
                  Odoo: {v.variant_odoo_product_id}
                </span>
              )}
            </div>
          ))
        ) : (
          <span className="text-gray-500 italic">لا توجد خيارات</span>
        ),
    },
    {
      key: "products_expiry_period_in_days",
      title: "صلاحية (يوم)",
      sortable: true,
      headerClassName: "min-w-[90px]",
      align: "center",
    },
    {
      ...
      render: (p) => (
        <span
          className={`font-semibold text-xs px-2 py-1 rounded-full ${p.products_is_active === 1 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
          {p.products_is_active === 1 ? "نشط" : "غير نشط"}
        </span>
      ),
    },
    {
