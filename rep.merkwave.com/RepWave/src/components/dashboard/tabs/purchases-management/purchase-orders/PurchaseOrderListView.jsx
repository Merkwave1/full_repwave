...
export default function PurchaseOrderListView({
  ...
  const getSupplierNameById = React.useCallback(
    (supplierId) => {
  ...
  const getWarehouseNameById = React.useCallback(
    (warehouseId) => {
  ...
  const columns = [
    {
      key: "purchase_orders_id",
      title: "رقم الأمر",
      render: (item) => (
        <span className="bg-blue-100 text-[#1F2937] text-sm px-3 py-1 rounded-full font-bold">
    ...
    ...(odooEnabled
      ? [
          {
            key: "purchase_orders_odoo_id",
            title: "Odoo ID",
            render: (item) =>
              item.purchase_orders_odoo_id ? (
      ...
      : []),
    {
      key: "supplier",
      title: "المورد",
      render: (item) => getSupplierNameById(item.purchase_orders_supplier_id),
      sortable: true,
      sortAccessor: (it) => getSupplierNameById(it.purchase_orders_supplier_id),
    },
    {
      key: "date",
      title: "التاريخ",
      render: (item) =>
        item.purchase_orders_order_date
          ? formatLocalDateTime(item.purchase_orders_order_date)
          : "–",
      sortable: true,
      sortKey: "purchase_orders_order_date",
    },
    {
      key: "total",
      title: "الإجمالي",
      render: (item) => formatMoney(item.purchase_orders_total_amount ?? 0),
      sortable: true,
      sortKey: "purchase_orders_total_amount",
      align: "right",
      headerAlign: "right",
      ...
      render: (item) => (
        <span
          className={`font-semibold text-xs px-2 py-1 rounded-full ${
            item.purchase_orders_status === "Ordered"
              ? "bg-yellow-100 text-yellow-800"
              : item.purchase_orders_status === "Received"
                ? "bg-green-100 text-green-800"
                : item.purchase_orders_status === "Partially Received"
                  ? "bg-green-100 text-green-800"
                  : "bg-red-100 text-red-800"
          }`}
        >
          {item.purchase_orders_status}
        </span>
      ),
      align: "center",
    },
    {
      key: "warehouse",
      title: "المخزن",
      render: (item) => getWarehouseNameById(item.purchase_orders_warehouse_id),
      sortable: true,
      sortAccessor: (it) =>
        getWarehouseNameById(it.purchase_orders_warehouse_id),
    },
    {
      key: "notes",
      title: "ملاحظات",
      render: (item) => item.purchase_orders_notes || "–",
    },
    {
      key: "actions",
      title: "الإجراءات",
      render: (item) => (
        <div className="flex items-center justify-center gap-2">
          ...
          {item.purchase_orders_status === "Draft" && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onEdit(item);
