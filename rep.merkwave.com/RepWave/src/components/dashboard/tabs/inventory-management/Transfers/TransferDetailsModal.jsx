...
function TransferDetailsModal({
  ...
  const getProductVariantDisplayName = useCallback(
    (productId, variantId) => {
  ...
  const getPackagingTypeDetails = useCallback(
    (packagingTypeId) => {
      return packagingTypes.find(
        (pt) => pt.packaging_types_id === packagingTypeId,
      );
    },
    [packagingTypes],
  );
  ...
  const normalizedInventoryItems = useMemo(() => {
    ...
    if (allInventoryItems && Array.isArray(allInventoryItems.data))
      return allInventoryItems.data;
    return [];
  }, [allInventoryItems]);
  ...
  useEffect(() => {
    console.log(
      "[TransferDetailsModal] normalizedInventoryItems count:",
      normalizedInventoryItems.length,
    );
    console.log(
      "[TransferDetailsModal] transfer source warehouse:",
      transfer?.transfer_source_warehouse_id,
    );
    console.log(
      "[TransferDetailsModal] transfer destination warehouse:",
      transfer?.transfer_destination_warehouse_id,
    );
    if (normalizedInventoryItems.length > 0) {
      console.log(
        "[TransferDetailsModal] Sample inventory item:",
        normalizedInventoryItems[0],
      );
    }
  }, [normalizedInventoryItems, transfer]);

  // Helper to get available quantity in a specific warehouse
  const getAvailableQuantityInSpecificWarehouse = useCallback(
    (productId, variantId, packagingTypeId, warehouseId) => {
      const relevantInventory = normalizedInventoryItems.filter((item) => {
        ...
        const matchPackaging =
          parseInt(item.packaging_type_id) === parseInt(packagingTypeId);
      ...
      // Debug log
      console.log(
        `[getAvailableQuantity] Checking: productId=${productId}, variantId=${variantId}, packagingId=${packagingTypeId}, warehouseId=${warehouseId}`,
      );
      console.log(
        `[getAvailableQuantity] Found ${relevantInventory.length} matching items`,
      );
      if (relevantInventory.length > 0) {
        console.log(
          `[getAvailableQuantity] Sample match:`,
          relevantInventory[0],
        );
      }

      const totalAvailable = relevantInventory.reduce(
        (sum, item) => sum + (parseFloat(item.inventory_quantity) || 0),
        0,
      );
  ...
  const groupedTransferItems = useMemo(() => {
    ...
    if (
      Array.isArray(editableItems) &&
      Array.isArray(products) &&
      Array.isArray(packagingTypes) &&
      Array.isArray(baseUnits) &&
      Array.isArray(normalizedInventoryItems)
    ) {
      editableItems.forEach((item) => {
        const product = products.find(
          (p) => p.products_id === item.products_id,
        );
        const packagingType = getPackagingTypeDetails(item.packaging_type_id);
        const baseUnit = product
          ? getBaseUnitName(product.products_unit_of_measure_id)
          : "غير معروف";

        const variantKey = item.variant_id
          ? `v-${item.variant_id}`
          : `p-${item.products_id}`;
        const variantDisplayName = getProductVariantDisplayName(
          item.products_id,
          item.variant_id,
        );
        ...
        const quantityInBaseUnit =
          (parseFloat(item.quantity) || 0) * conversionFactor; // Use item.quantity
        ...
        grouped[variantKey].packaging_details.push({
          ...
          packaging_type_name:
            packagingType?.packaging_types_name || "غير معروف",
    ...
    transfer.transfer_destination_warehouse_id,
    transfer.transfer_source_warehouse_id,
    getAvailableQuantityInSpecificWarehouse,
    getPackagingTypeDetails,
    getBaseUnitName,
      ...
      if (
        ...
        if (product && packagingType) {
          ...
          options.push({
            ...
            label: label,
            item_details: {
              // Store necessary details to reconstruct the item
  ...
  const handleQuantityChange = (inventoryId, newQuantity) => {
    ...
    setItemErrors((prevErrors) => ({
      ...prevErrors,
      [`qty_${inventoryId}`]: null,
    }));
  };

  const handleAddItem = (selectedOptionValue) => {
    const selectedItem = availableInventoryOptions.find(
      (opt) => opt.value === selectedOptionValue,
    );
    if (
      selectedItem &&
      !editableItems.some((item) => item.inventory_id === selectedItem.value)
    ) {
      setEditableItems((prevItems) => [
        ...prevItems,
        { ...selectedItem.item_details, quantity: 1 },
      ]); // Add with default quantity 1
    } else if (selectedItem) {
      setGlobalMessage({
        type: "warning",
        message: "هذا العنصر موجود بالفعل في التحويل.",
      });
    }
  };
  ...
  const handleRemoveItem = (inventoryIdToRemove) => {
    setEditableItems((prevItems) =>
      prevItems.filter((item) => item.inventory_id !== inventoryIdToRemove),
    );
    setItemErrors((prevErrors) => {
  ...
  const handleSaveChanges = async () => {
    if (!validateEditForm()) {
      setGlobalMessage({
        type: "error",
        message: "الرجاء تصحيح الأخطاء في عناصر التحويل.",
      });
      return;
    }
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: responseMessage || "تم تحديث التحويل بنجاح!",
      });
      ...
      setGlobalMessage({
        type: "error",
        message: error.message || "فشل في تحديث التحويل.",
      });
    } finally {
      setIsSaving(false);
    }
    ...
    >
      ...
      <div className="p-3 sm:p-6 flex-grow overflow-y-auto bg-gray-50">
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <DetailItem
              ...
              label="المخزن المصدر"
              value={getWarehouseName(transfer.transfer_source_warehouse_id)}
            />
            <DetailItem
              icon={<BuildingOffice2Icon />}
              label="المخزن الوجهة"
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h4 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">
            عناصر التحويل (مجمعة حسب المنتج/الخيار)
          </h4>
          {editableItems.length === 0 && (
            <p className="text-center text-gray-600 py-4">
              لا توجد عناصر لهذا التحويل.
            </p>
          )}
          {itemErrors.general && (
          ...
          {groupedTransferItems.length > 0 ? (
            <div className="space-y-4">
              {groupedTransferItems.map((groupedItem, index) => (
                <div
                  key={groupedItem.variant_key || index}
                ...
                >
                  <h5 className="text-lg font-semibold text-gray-700 mb-3 flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-gray-600">
                      إجمالي الكمية:{" "}
                      {englishDecimalFormatter.format(
                        groupedItem.total_quantity_in_base_unit || 0,
                      )}{" "}
                      {groupedItem.base_unit_name}
                    </span>
                  </h5>
                  <div className="col-span-full">
                    ...
                    <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                      {groupedItem.packaging_details.map((pkg, pkgIndex) => {
                        ...
                        return (
                          <li
                            key={pkg.inventory_id || pkgIndex}
                            className="flex flex-wrap items-center justify-between py-2 border-b border-gray-200 last:border-b-0"
                          >
                            <div className="flex items-center w-full sm:w-auto mb-2 sm:mb-0">
                              <span className="flex-none text-gray-700 mr-2">
                                {pkg.packaging_type_name} (
                                {englishDecimalFormatter.format(
                                  pkg.quantity_in_base_unit || 0,
                                )}{" "}
                                {groupedItem.base_unit_name}):
                              </span>
                              {isEditing ? (
                                <NumberInput
                                  ...
                                  onChange={(val) =>
                                    handleQuantityChange(pkg.inventory_id, val)
                                  }
                              ...
                              {isEditing && (
                                <button
                                  onClick={() =>
                                    handleRemoveItem(pkg.inventory_id)
                                  }
                            ...
                            <div className="flex flex-wrap items-center w-full sm:w-auto sm:justify-end">
                              <span
                                className={`text-xs mr-2 flex items-center ${sourceAvailabilityClass}`}
                              >
                                {exceedsSource && (
                                ...
                                (متاح في المصدر:{" "}
                                {englishDecimalFormatter.format(
                                  pkg.available_in_source || 0,
                                )}{" "}
                                {pkg.packaging_type_name})
                              </span>
                              <span
                                className={`text-xs flex items-center ${destinationAvailabilityClass}`}
                              >
                                {/* No warning icon for destination availability, as per request focus on source */}
                                (متاح في الوجهة:{" "}
                                {englishDecimalFormatter.format(
                                  pkg.available_in_destination || 0,
                                )}{" "}
                                {pkg.packaging_type_name})
                              </span>
                            </div>
          ...
          {isEditing ? (
            <>
              <button
                ...
                className="w-full sm:w-auto px-4 py-3 sm:px-6 sm:py-2.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-150 ease-in-out shadow-md flex items-center justify-center gap-2 text-base sm:text-sm"
                disabled={isSaving}
              >
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 py-3 sm:px-6 sm:py-2.5 border border-gray-300 rounded-md shadow-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out text-base sm:text-sm"
                disabled={isSaving}
              >
                إلغاء
              </button>
            ...
            <>
              ...
              {transfer.status === "Pending" && (
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-3 sm:px-6 sm:py-2.5 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out shadow-md flex items-center justify-center gap-2 text-base sm:text-sm"
                >
