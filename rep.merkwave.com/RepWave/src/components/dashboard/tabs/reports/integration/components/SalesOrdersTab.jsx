...
const SalesOrdersTab = ({ onStatsUpdate }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      align: 'center',
      render: (item) => <span className="font-semibold text-blue-600">{item.php_order_id}</span>
    },
    {
      key: 'odoo_order_id',
      title: 'Odoo Order ID',
    ...
    },
    {
      ...
      render: (item) => item.odoo_invoice_id ? (
        <span className="font-mono bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-sm">{item.odoo_invoice_id}</span>
      ) : (
        <span className="text-gray-400">-</span>
      )
    },
