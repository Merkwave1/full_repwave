...
function ClientsTab() {
      ...
      currentFilteredClients = currentFilteredClients.filter(
        (client) =>
          ...
          client.clients_contact_phone_1
            ?.toLowerCase()
            .includes(lowerCaseSearchTerm) ||
          client.clients_contact_phone_2
            ?.toLowerCase()
            .includes(lowerCaseSearchTerm) ||
          (odooEnabled &&
  ...
  const handleAddClient = async (newClientData) => {
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: message || "تم إضافة العميل بنجاح!",
      });
      setCurrentView("list");
  ...
  const handleUpdateClient = async (updatedClientData) => {
    try {
      ...
      const message = await updateClient(
        updatedClientData.clients_id,
        updatedClientData,
      );
      setGlobalMessage({
        type: "success",
        message: message || "تم تحديث العميل بنجاح!",
      });
      setCurrentView("list");
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: message || "تم حذف العميل بنجاح!",
      });
      setCurrentView("list");
                    ...
                    },
                    {
                      ...
                      render: (c) => (
                        ...
                        >
                          ...
                          c.clients_contact_phone_1.trim() !== ""
                            ? c.clients_contact_phone_1
                            : "غير محدد"}
                        </div>
                      ),
