...
export default function UpdatePurchaseReturnFormEnhanced({
              ...
              setFormData(prev => ({
                ...
                items: prev.items.map(item => {
                  ...
                  if (returnableInfo) {
                    ...
                    const adjustedReceivedAvailable = returnableInfo.available_to_return_received + currentReturnQty;
                    const adjustedNonReceivedAvailable = returnableInfo.available_to_return_not_received;
                    ...
                    return {
                      ...
                      available_to_return_not_received: adjustedNonReceivedAvailable,
                      total_available_to_return: adjustedReceivedAvailable + adjustedNonReceivedAvailable
                    };
                  }
                  return item;
                })
        ...
        if (field === 'purchase_order_item_id' && value) {
          ...
          if (returnableInfo) {
            Object.assign(updatedItem, {
              ...
              available_to_return_received: returnableInfo.available_to_return_received,
              available_to_return_not_received: returnableInfo.available_to_return_not_received,
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmMessage = `هل أنت متأكد من تحديث مرتجع الشراء؟\n\n` +
                          ...
                          <div className="text-xs mt-1">
                            {item.available_to_return_not_received > 0 && item.available_to_return_received > 0 ? (
                              <span className="text-green-600">
                                متاح لارجاع {item.available_to_return_not_received} لم يتم استلامهم +{item.available_to_return_received} تم استلامهم
                              </span>
                            ) : item.available_to_return_received > 0 ? (
                              <span className="text-blue-600">
                                متاح للارجاع: {item.available_to_return_received} تم استلامهم
                              </span>
                            ) : item.available_to_return_not_received > 0 ? (
                              <span className="text-orange-600">
                                متاح للارجاع: {item.available_to_return_not_received} لم يتم استلامهم
                              </span>
                            ) : (
                              <span className="text-green-600">
