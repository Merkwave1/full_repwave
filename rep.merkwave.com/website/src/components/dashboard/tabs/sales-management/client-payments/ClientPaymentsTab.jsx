// src/components/dashboard/tabs/sales-management/client-payments/ClientPaymentsTab.jsx
...
import { getClientPayments, deleteClientPayment } from '../../../../../apis/client_payments';
import { getPaymentMethods } from '../../../../../apis/payment_methods';
import { getSafes } from '../../../../../apis/safes';
import { getAllClients } from '../../../../../apis/clients';

import AddClientPaymentForm from './AddClientPaymentForm';
import UpdateClientPaymentForm from './UpdateClientPaymentForm';
import ClientPaymentDetailsModal from './ClientPaymentDetailsModal';
...
// List View
const ClientPaymentListView = ({ payments, loading, error, searchTerm, formatDate, formatCurrency, onEdit, onView, onDelete, odooEnabled }) => {
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden animate-fadeIn">
      <div className="bg-gradient-to-r from-indigo-100 to-blue-100 px-6 py-4 border-b border-gray-300 flex items-center justify-between">
        <div className="text-sm text-gray-800">
          إجمالي مدفوعات العملاء: <span className="font-bold text-indigo-700 ml-1">{payments.length}</span>
        </div>
        {searchTerm && (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="w-16 px-4 py-3 text-center text-xs font-bold text-gray-700 uppercase border-r border-gray-200">ID</th>
              {odooEnabled && <th className="w-16 px-4 py-3 text-center text-xs font-bold text-gray-700 uppercase border-r border-gray-200">ODOO</th>}
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {payments.map((item, idx) => (
              <tr key={item.client_payments_id || item.id || idx} className="hover:bg-gray-50 transition-all duration-150">
                <td className="text-center px-4 py-3 border-r border-gray-200">
                  <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-1 rounded-full font-semibold">#{item.client_payments_id || item.payments_id || item.id}</span>
                </td>
                {odooEnabled && (
                  <td className="text-center px-4 py-3 border-r border-gray-200">
                    {item.payments_odoo_payment_id ? (
                      <span className="bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded-full font-semibold">{item.payments_odoo_payment_id}</span>
                    ) : (
                ...
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200">{formatDate(item.client_payments_date || item.payment_date)}</td>
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200 font-semibold">{formatCurrency(item.client_payments_amount || item.amount)}</td>
                <td className="px-4 py-3 text-gray-600 text-sm border-r border-gray-200">{item.client_payments_notes || item.notes || 'لا توجد ملاحظات'}</td>
        ...
        {payments.length === 0 && <div className="text-center py-8 text-gray-500">لا توجد مدفوعات عملاء</div>}
      </div>
    </div>
  );
};
...
export default function ClientPaymentsTab() {
  ...
  const loadPayments = useCallback(async () => {
    ...
    try {
      ...
      const list = res.client_payments || res.data?.client_payments || res.data || res || [];
      setPayments(Array.isArray(list) ? list : []);
    } catch (e) {
      setError(e.message || 'Error loading client payments');
      setGlobalMessage({ type: 'error', message: 'فشل في تحميل مدفوعات العملاء.' });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadPayments();
    loadContext();
  }, [loadPayments, loadContext]);

  useEffect(() => {
    setChildRefreshHandler(() => loadPayments);
  ...
  const filtered = useMemo(() => {
    ...
    if (term) {
      list = list.filter((i) =>
        ...
        (i.client_payments_notes || i.notes || '').toLowerCase().includes(term) ||
  ...
  const handleEdit = (payment) => { setSelectedPayment(payment); setCurrentView('edit'); };
  const handleView = (payment) => { setSelectedPayment(payment); setCurrentView('details'); };
  const handleDelete = (payment) => { setPaymentToDelete(payment); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    try {
      await deleteClientPayment(paymentToDelete.client_payments_id || paymentToDelete.id);
  ...
  const handleCloseModal = () => { setCurrentView('list'); setSelectedPayment(null); };

  const renderContent = () => {
    if (currentView === 'add') {
      return (
        <AddClientPaymentForm onClose={handleCloseModal} onSubmit={handleFormSubmit} safes={safes} clients={clients} paymentMethods={paymentMethods} />
      );
    }
    if (currentView === 'edit') {
      return (
        <UpdateClientPaymentForm onClose={handleCloseModal} onSubmit={handleFormSubmit} safes={safes} clients={clients} paymentMethods={paymentMethods} payment={selectedPayment} />
      );
    }
    if (currentView === 'details') {
      return (
        <ClientPaymentDetailsModal onClose={handleCloseModal} payment={selectedPayment} clients={clients} safes={safes} paymentMethods={paymentMethods} />
      );
    }
    ...
    return (
      <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
          <h3 className="text-2xl font-bold text-gray-800 flex-none">مدفوعات العملاء</h3>
          <div className="relative flex-grow mx-4">
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="ابحث عن مدفوعات العملاء..." className="w-full pl-4 pr-10 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" dir="rtl" />
            {searchTerm && (
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 mb-6">
          ...
          <div className="flex items-center gap-3 mt-4">
            <button onClick={loadPayments} className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700">تطبيق التصفية</button>
        ...
        <ClientPaymentListView payments={filtered} loading={loading} error={error} searchTerm={searchTerm} formatDate={formatDate} formatCurrency={formatCurrency} onEdit={handleEdit} onView={handleView} onDelete={handleDelete} odooEnabled={odooEnabled} />
        ...
        {deleteModalOpen && (
          <DeleteConfirmationModal
            ...
            itemName={`دفعة عميل #${paymentToDelete?.client_payments_id || paymentToDelete?.id || ''}`}
            deleteLoading={false}
          />
        )}
      </>
