...
const AddClientPaymentForm = ({ onClose, onSubmit, safes = [], clients = [], paymentMethods = [], title, extraHeaderRight }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return;
    const selectedClient = (clients || []).find(c => String(c.clients_id || c.id) === String(formData.client_id));
    const clientName = selectedClient?.clients_company_name || selectedClient?.company_name || selectedClient?.name || 'هذا العميل';
    ...
    const confirmSave = window.confirm(`هل تريد حفظ دفعة بقيمة ${amountDisplay} للعميل ${clientName}؟`);
