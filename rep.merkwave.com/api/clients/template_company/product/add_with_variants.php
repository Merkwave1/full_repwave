...
try {
    ...
    $products_has_tax                  = $_POST['products_has_tax'] ?? 1;
    ...
    // Track created variant IDs for Odoo sync
    $created_variant_ids = [];
    ...
    try {
        ...
        // 1. Insert into `products` table (basic version without tax fields for now)
        ...
        // Updated bind_param - basic fields only
        ...
        if (!empty($variants_data)) {
            ...
            foreach ($variants_data as $index => $variant) {
                ...
                $stmt_variant->bind_param("issssddddisid", 
                    $new_products_id,
                ...
                // Store variant ID for Odoo sync after commit
                $created_variant_ids[] = $new_variant_id;
            }
        ...
        // Sync variants to Odoo (after successful commit, non-blocking)
        $odoo_sync_results = [];
        if (!empty($created_variant_ids)) {
            foreach ($created_variant_ids as $variant_id) {
                try {
                    ...
                    if ($odoo_product_id) {
                        ...
                        error_log("Variant $variant_id synced to Odoo as product $odoo_product_id");
                    } else {
                        $odoo_sync_results[] = [
                            'variant_id' => $variant_id,
                ...
                } catch (Exception $odoo_e) {
                    error_log("Odoo sync failed for variant $variant_id: " . $odoo_e->getMessage());
                    $odoo_sync_results[] = [
                        'variant_id' => $variant_id,
                        'odoo_product_id' => null,
                        'status' => 'failed',
                        'error' => $odoo_e->getMessage()
                    ];
                }
            }
        }
