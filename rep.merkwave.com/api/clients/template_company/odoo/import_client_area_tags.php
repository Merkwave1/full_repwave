<?php
/**
 * Import Client Area Tags from Odoo
 * 
 * Imports contact.area.tag (Area Tags) from Odoo to client_area_tags table in Rep
 * Uses Odoo area tag ID as Rep area tag ID for synchronization.
 */
...
try {
    ...
    // Fetch area tags from Odoo (contact.area.tag custom model)
    $tags = importOdooCall($odoo_url, 'contact.area.tag', 'search_read', $cookie_file,
        [[]],
    ...
    if (!is_array($tags) || empty($tags)) {
        print_failure('No area tags found in Odoo');
        exit;
    }
    
    // Get existing tags
    $existing = [];
    $stmt = $conn->prepare("SELECT client_area_tag_id FROM client_area_tags");
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $existing[(int)$row['client_area_tag_id']] = true;
    }
    $stmt->close();
    ...
    foreach ($tags as $index => $tag) {
        try {
            ...
            if (isset($existing[$id])) {
                // Update
                if (!$dry_run) {
                    $stmt = $conn->prepare("UPDATE client_area_tags SET client_area_tag_name = ?, client_area_tag_sort_order = ? WHERE client_area_tag_id = ?");
                ...
                // Insert
                if (!$dry_run) {
                    $stmt = $conn->prepare("INSERT INTO client_area_tags (client_area_tag_id, client_area_tag_name, client_area_tag_sort_order) VALUES (?, ?, ?)");
            ...
            $stats['failed']++;
            $stats['details'][] = ['action' => 'failed', 'id' => $tag['id'] ?? 0, 'error' => $e->getMessage()];
        }
    }
    
    $message = sprintf('تم استيراد مناطق العملاء: %d جديد، %d محدث، %d فشل', $stats['created'], $stats['updated'], $stats['failed']);
