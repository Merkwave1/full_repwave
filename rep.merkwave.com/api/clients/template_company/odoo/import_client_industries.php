<?php
/**
 * Import Client Industries from Odoo
 * 
 * Imports res.partner.industry from Odoo to client_industries table in Rep
 * Uses Odoo industry ID as Rep industry ID for synchronization.
 */
...
try {
    ...
    // Fetch industries from Odoo
    $industries = importOdooCall($odoo_url, 'res.partner.industry', 'search_read', $cookie_file,
        [[]],
        ['context' => ['active_test' => false],

        'fields' => ['id', 'name', 'full_name'], 'order' => 'name ASC']
    );
    
    if (file_exists($cookie_file)) unlink($cookie_file);
    
    if (!is_array($industries) || empty($industries)) {
        print_failure('No industries found in Odoo');
        exit;
    }
    
    // Get existing industries
    $existing = [];
    $stmt = $conn->prepare("SELECT client_industries_id FROM client_industries");
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $existing[(int)$row['client_industries_id']] = true;
    }
    $stmt->close();
    ...
    foreach ($industries as $index => $industry) {
        try {
            ...
            if (isset($existing[$id])) {
                // Update
                if (!$dry_run) {
                    $stmt = $conn->prepare("UPDATE client_industries SET client_industries_name = ?, client_industries_sort_order = ? WHERE client_industries_id = ?");
                ...
                // Insert
                if (!$dry_run) {
                    $stmt = $conn->prepare("INSERT INTO client_industries (client_industries_id, client_industries_name, client_industries_sort_order) VALUES (?, ?, ?)");
            ...
            $stats['failed']++;
            $stats['details'][] = ['action' => 'failed', 'id' => $industry['id'] ?? 0, 'error' => $e->getMessage()];
        }
    }
    
    $message = sprintf('تم استيراد الصناعات: %d جديد، %d محدث، %d فشل', $stats['created'], $stats['updated'], $stats['failed']);
    ...
    print_success($message, [
        ...
        'failed' => $stats['failed'],
        'total_from_odoo' => count($industries),
        'dry_run' => $dry_run,
