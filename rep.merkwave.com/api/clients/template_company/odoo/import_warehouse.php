<?php
/**
 * Import Warehouses from Odoo
 * 
 * Imports stock.warehouse from Odoo to warehouse table in Rep
 ...
 * - warehouse_name: name
 * - warehouse_code: code
 * - warehouse_odoo_warehouse_id: Odoo warehouse ID
 */
...
try {
    ...
    if (empty($warehouses)) {
        echo json_encode([
            'status' => 'success',
            'message' => 'لم يتم العثور على مستودعات في Odoo',
            'data' => [
                'imported' => 0,
                'updated' => 0
            ]
    ...
    foreach ($warehouses as $wh) {
        ...
        $warehouse_status = $wh['active'] ? 'Active' : 'Inactive';
        
        // Check if exists by ID
        $stmt = $conn->prepare("SELECT warehouse_id FROM warehouse WHERE warehouse_id = ?");
        ...
        if ($exists) {
            ...
            if ($stmt->execute()) {
                ...
                $errors[] = "Failed to update warehouse ID $warehouse_id: " . $stmt->error;
            }
            $stmt->close();
        } else {
            ...
            if ($stmt->execute()) {
                ...
                $errors[] = "Failed to insert warehouse ID $warehouse_id: " . $stmt->error;
            }
            $stmt->close();
        }
    }
    ...
    echo json_encode([
        'status' => 'success',
        'message' => "تم استيراد المستودعات: $imported جديد، $updated محدث",
        'data' => [
            'imported' => $imported,
            'updated' => $updated,
            'total_from_odoo' => count($warehouses),
            'errors' => $errors
        ]
    ]);
