...
try {
    ...
    // 1. Status must be 'Invoiced' only (business rule: only deliver invoiced orders)
    $where_clauses[] = "so.sales_orders_status = 'Invoiced'";

    // 2. Delivery status must be 'Not_Delivered', 'Processing_Delivery', 'Partially_Delivered', NULL, or empty
    // (aligned with get_pending_orders.php to include all non-delivered orders)
    $where_clauses[] = "(so.sales_orders_delivery_status IN ('Not_Delivered','Processing_Delivery','Partially_Delivered') 
