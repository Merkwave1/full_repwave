...
try {
    ...
    if (!empty($warehouse_type)) {
        $allowedTypes = ['Main', 'Van', 'main', 'van', 'mobile'];
        if (!in_array($warehouse_type, $allowedTypes)) {
            print_failure("Error: Invalid warehouse type. Must be 'Main' or 'Van'.");
            exit;
        }
        // Normalize to title case
        $warehouse_type = ucfirst(strtolower($warehouse_type));
        if ($warehouse_type === 'Mobile') $warehouse_type = 'Van'; // map mobile -> Van
