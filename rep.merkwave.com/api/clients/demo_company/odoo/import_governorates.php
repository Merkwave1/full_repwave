<?php
/**
 * Import Governorates/States from Odoo
 * 
 * Imports res.country.state from Odoo to governorates table in Rep
 * Uses Odoo state ID as Rep governorate ID for synchronization.
 * Requires countries to be جديد first.
 */
...
try {
    ...
    // Fetch states from Odoo
    $states = importOdooCall($odoo_url, 'res.country.state', 'search_read', $cookie_file,
        [[]],
        ['context' => ['active_test' => false],

        'fields' => ['id', 'name', 'code', 'country_id'], 'order' => 'country_id, name ASC']
    );
    
    if (file_exists($cookie_file)) unlink($cookie_file);
    
    if (!is_array($states) || empty($states)) {
        print_failure('No states/governorates found in Odoo');
        exit;
    }
    
    // Get existing governorates
    $existing = [];
    $stmt = $conn->prepare("SELECT governorates_id FROM governorates");
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $existing[(int)$row['governorates_id']] = true;
    }
    $stmt->close();
    ...
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $countries[(int)$row['countries_id']] = true;
    }
    $stmt->close();
    ...
    foreach ($states as $index => $state) {
        try {
            ...
            $country_id = is_array($state['country_id']) ? $state['country_id'][0] : $state['country_id'];
            
            // Skip if country doesn't exist in Rep
            ...
            if (isset($existing[$id])) {
                // Update
                if (!$dry_run) {
                    $stmt = $conn->prepare("UPDATE governorates SET governorates_country_id = ?, governorates_name_ar = ?, governorates_name_en = ?, governorates_sort_order = ? WHERE governorates_id = ?");
                    $stmt->bind_param('issii', $country_id, $name, $name, $index, $id);
                    $stmt->execute();
                    $stmt->close();
                }
                $stats['updated']++;
                ...
                // Insert
                if (!$dry_run) {
                    $stmt = $conn->prepare("INSERT INTO governorates (governorates_id, governorates_country_id, governorates_name_ar, governorates_name_en, governorates_sort_order) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param('iissi', $id, $country_id, $name, $name, $index);
                    $stmt->execute();
                    $stmt->close();
                }
                $stats['created']++;
            ...
            $stats['failed']++;
            $stats['details'][] = ['action' => 'failed', 'id' => $state['id'] ?? 0, 'error' => $e->getMessage()];
        }
    }
    
    $message = sprintf('تم استيراد المحافظات: %d جديد، %d محدث، %d تخطي، %d فشل', 
