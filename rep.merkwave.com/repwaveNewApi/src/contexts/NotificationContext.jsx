...
import { useSignalRNotifications } from "../hooks/useSignalRNotifications.js";
...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchNotifications = useCallback(
    async ({ append = false, page = null, ...params } = {}) => {
      ...
      try {
        ...
        // .NET format: api.get auto-unwraps PagedResult → plain array
        if (Array.isArray(response)) {
          const fetchedNotifications = response;
          setNotifications((prev) =>
            append ? [...prev, ...fetchedNotifications] : fetchedNotifications,
          );
          const unread = fetchedNotifications.filter(
            (n) => !(n.NotificationsIsRead ?? n.is_read ?? false),
          ).length;
          setUnreadCount(unread);
          setPendingOperations({});
          setPendingOperationsTotal(0);
          setPagination({
            current_page: nextPage,
            per_page: DEFAULT_PAGE_SIZE,
            total_items: fetchedNotifications.length,
            total_pages: Math.max(
              1,
              Math.ceil(fetchedNotifications.length / DEFAULT_PAGE_SIZE),
            ),
          });
          return;
        }
  ...
  const fetchUnreadCount = useCallback(async () => {
    ...
    try {
      ...
      // .NET format: api.get auto-unwraps PagedResult → plain array of unread notifications
      if (Array.isArray(response)) {
  ...
  // On login: always fetch full notifications (not just count)
  useEffect(() => {
  ...
  // Real-time SignalR: refresh notifications when server pushes a new one
