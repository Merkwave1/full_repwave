...
const isAuthorizationError = (message) => {
  ...
  // Exclude role-based access denials — these are permission errors, not session errors
  if (lowerMessage.includes('admin access required')) return false;
  return lowerMessage.includes('invalid session') ||
...
const coreApiClient = async (url, options = {}) => {
  ...
  // If url is already an absolute path or full URL, use it directly
