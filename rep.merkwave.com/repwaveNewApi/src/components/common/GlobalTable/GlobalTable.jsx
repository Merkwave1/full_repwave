...
function SortIndicator({ active, direction }) {
  if (!active) {
    return (
      <div className="flex flex-col items-center text-[#1A0F35]">
  ...
  return direction === "asc" ? (
    <ChevronUpIcon className="h-5 w-5 text-[#C4A8F0]" />
  ) : (
    <ChevronDownIcon className="h-5 w-5 text-[#C4A8F0]" />
  );
}
...
export default function GlobalTable({
  ...
  const renderHeader = () => (
    <thead className="bg-white sticky top-0 z-10 border-b border-gray-200">
      <tr className="relative after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-[#C4A8F0]/40">
          ...
          return (
            <th
              ...
              className={joinClasses(
                "px-6 py-4 text-xs font-semibold text-[#1A0F35] whitespace-nowrap",
                alignMap[col.align],
    ...
    <tbody>
      {displayedData.map((item, index) => {
        ...
        const rowClasses = joinClasses(
          ...
          "hover:bg-[#C4A8F0]/10 hover:shadow-[inset_4px_0_0_#8DD8F5]",
          ...
          return (
            <div
              ...
              className={joinClasses(
                ...
                onRowClick && "cursor-pointer active:bg-[#f5f3ff]/60",
              )}
              onClick={onRowClick ? () => onRowClick(item) : undefined}
            >
  ...
  const renderFooter = () => {
    ...
    return (
      <tfoot className="bg-gray-50/70 border-t border-gray-200">
        <tr>
          {normalizedColumns.map((col, i) => (
            <td key={i} className="px-6 py-3 text-sm font-bold text-[#1A0F35]">
    ...
    <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden">
      {summaryVisible && (
        <div className="hidden md:flex px-5 py-3 border-b border-gray-100 text-sm text-gray-700 justify-between">
          <div>
            إجمالي العناصر:
            <span className="font-bold text-[#1A0F35] mr-2">
          ...
          {searchTerm && (
            <div className="text-gray-500">
              نتائج البحث:{" "}
              <span className="font-medium text-[#1A0F35]">{searchTerm}</span>
            </div>
          )}
        </div>
      )}
          ...
          <div className="md:hidden w-full overflow-hidden">
            {summaryVisible && (
              <div className="px-3 pt-3 text-xs text-gray-500">
                إجمالي:{" "}
                <span className="font-bold text-[#1A0F35]">
                  {totalCount ?? data.length}
                </span>
              </div>
            )}
