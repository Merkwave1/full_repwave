...
import {
  ...
  getBreakLogs,
} from "../../../../../../apis/representative_attendance";
...
export default function AttendanceTab({ representatives = [] }) {
    ...
    try {
      ...
      if (cachedUsers && Array.isArray(cachedUsers)) {
        return cachedUsers.filter(
          (u) => u.users_role === "rep" && u.users_status === 1,
        );
      }
    } catch (error) {
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (selectedRep) {
      const user = getUsers().find(
        (u) => String(u.users_id) === String(selectedRep),
      );
      chips.push({
        key: "representative",
        label: "المندوب",
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "مدة العمل",
      render: (row) => (
        <span className="font-medium text-[#8B5FD6]">
      ...
      render: (row) => {
        const statusMap = {
          ...
          ClockedOut: {
            label: "تسجيل خروج",
            color:
              "p-1.5 rounded-full text-sky-700 bg-sky-100 hover:bg-sky-500 hover:text-white hover:shadow-[0_0_12px_rgba(56,189,248,0.45)]transition-all duration-200 hover:scale-110",
          },
        };
        const status = statusMap[row.attendance_status] || {
  ...
  return (
    <div className="p-6">
      ...
      <CustomPageHeader
        ...
        icon={<ClockIcon className="h-8 w-8 text-[#1A0F35]" />}
        ...
        actionButton={
          <PrintableTable
            ...
            printButtonClassName="bg-white text-[#8B5FD6] hover:bg-[#f5f3ff] px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-200 shadow-md hover:shadow-lg font-bold"
          />
        }
      />
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {selectedAttendance.shift_start_time || "-"}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <p className="text-gray-900">
                  {selectedAttendance.shift_end_time || "لم ينتهي بعد"}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <p className="font-semibold text-[#8B5FD6]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900 text-sm">
                    {selectedAttendance.start_latitude &&
                    selectedAttendance.start_longitude
                ...
                <div>
                  ...
                  <p className="text-gray-900 text-sm">
                    {selectedAttendance.end_latitude &&
                    selectedAttendance.end_longitude
