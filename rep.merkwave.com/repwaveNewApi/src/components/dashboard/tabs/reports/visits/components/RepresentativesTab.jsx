...
const RepresentativesTab = ({ data }) => {
  ...
  const formatCurrency = (amount, options) =>
    formatAppCurrency(toNumber(amount), options);
  ...
  const getTotalRevenue = () => {
    return data.reduce(
      (sum, rep) => sum + toNumber(rep.total_sales_from_visits),
      0,
    );
  };
  ...
  const getOverallCompletionRate = () => {
    ...
    const totalCompleted = data.reduce(
      (sum, rep) => sum + (rep.completed_visits || 0),
      0,
    );
    return totalVisits > 0
      ? ((totalCompleted / totalVisits) * 100).toFixed(1)
      : 0;
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="flex-1 min-w-0 px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-xl bg-white text-sm sm:text-base text-gray-700 font-medium shadow-sm hover:border-green-400 focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent transition-all duration-200"
          >
            ...
            <option value="total_sales_from_visits">
              المبيعات من الزيارات
            </option>
          ...
          <button
            ...
            className="shrink-0 flex items-center px-3 sm:px-4 py-2 sm:py-3 bg-white border border-gray-300 rounded-xl shadow-sm hover:shadow-md hover:bg-gray-50 transition-all duration-200 text-sm font-medium text-gray-700"
          >
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6">
          <div className="text-center">
            <div className="text-2xl sm:text-3xl font-bold text-[#8B5FD6] mb-1 sm:mb-2">
              {data.length}
            </div>
          ...
          <div className="text-center">
            <div className="text-base md:text-2xl sm:text-3xl font-bold text-orange-600 mb-1 sm:mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6">
        {sortedData.map((rep) => {
          ...
          return (
            <div
              key={rep.users_id}
              className="bg-white rounded-lg border border-gray-200 p-3 sm:p-6 hover:shadow-lg transition-shadow"
            >
              {/* Rep Header */}
              <div className="flex items-start justify-between mb-3 sm:mb-4 gap-2">
                <div className="flex items-center min-w-0">
                  <div className="h-10 w-10 sm:h-12 sm:w-12 bg-[#EDE7FF] rounded-full flex items-center justify-center shrink-0">
                    <UserIcon className="h-5 w-5 sm:h-6 sm:w-6 text-[#8B5FD6]" />
                  </div>
                  <div className="mr-2 sm:mr-4 min-w-0">
                    ...
                    <div className="flex items-center text-xs sm:text-sm text-gray-500 truncate">
                      <EnvelopeIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1 shrink-0" />
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-3 sm:mb-4">
                <div className="bg-gray-50 rounded-lg p-2 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-lg sm:text-2xl font-bold text-[#8B5FD6]">
                ...
                <div className="bg-gray-50 rounded-lg p-2 sm:p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <PlayCircleIcon className="h-6 w-6 sm:h-8 sm:w-8 text-yellow-400" />
                  </div>
                </div>
              ...
              <div className="space-y-2 sm:space-y-3 pt-3 sm:pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-gray-600 flex items-center">
                    <UserGroupIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1" />
                    العملاء المزارين
                  </span>
                ...
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-gray-600 flex items-center">
                    <ClockIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1" />
                    متوسط مدة الزيارة
                  </span>
                  <span className="font-medium text-gray-900">
                ...
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-gray-600 flex items-center">
                    <CalendarDaysIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1" />
                    زيارات اليوم
                  </span>
                  <span className="font-medium text-gray-900">
                ...
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-gray-600 flex items-center">
                    <DocumentTextIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1" />
                    طلبات من الزيارات
                  </span>
                  <span className="font-medium text-gray-900">
                ...
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-gray-600 flex items-center">
                    <CurrencyDollarIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 ml-1" />
                    مبيعات من الزيارات
                  </span>
                  <span className="font-medium text-gray-900">
      ...
      {data.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
            ...
            <div className="bg-green-50 border border-[#C4A8F0] rounded-lg p-4">
              <h4 className="font-medium text-green-800 mb-2">
                أعلى معدل إنجاز
              </h4>
              {(() => {
                const topRep = [...data].sort((a, b) => {
                  const aRate = getCompletionRate(
                    a.completed_visits,
                    a.total_visits,
                  );
                  const bRate = getCompletionRate(
                    b.completed_visits,
                    b.total_visits,
                  );
                  return bRate - aRate;
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              {(() => {
                const topRep = [...data].sort(
                  (a, b) =>
                    (b.total_sales_from_visits || 0) -
                    (a.total_sales_from_visits || 0),
                )[0];
                return (
                  <div>
