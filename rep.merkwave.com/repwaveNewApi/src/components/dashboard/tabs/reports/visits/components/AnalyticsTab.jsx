      ...
      {dailyData && dailyData.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center mb-4">
            <CalendarDaysIcon className="h-6 w-6 text-[#8B5FD6] ml-2" />
      ...
      {hourlyData && hourlyData.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center mb-4">
            ...
            <h3 className="text-lg font-medium text-gray-900">
              توزيع الزيارات حسب الساعة (آخر 7 أيام)
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {hourlyData.map((hour) => (
              ...
              >
                <div className="text-lg font-bold text-[#8B5FD6]">
                  {hour.total_visits || 0}
                </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {dailyData.reduce(
                    (sum, day) => sum + (day.total_visits || 0),
                    0,
                  )}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-green-600">
                  {dailyData.reduce(
                    (sum, day) => sum + (day.completed_visits || 0),
                    0,
                  )}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-red-600">
                  {dailyData.reduce(
                    (sum, day) => sum + (day.cancelled_visits || 0),
                    0,
                  )}
                </span>
              </div>
