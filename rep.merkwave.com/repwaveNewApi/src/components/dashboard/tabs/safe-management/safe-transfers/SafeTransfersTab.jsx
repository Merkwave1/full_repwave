...
export default function SafeTransfersTab() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAllSafes(Array.isArray(safesResponse) ? safesResponse : safesResponse?.safes || safesResponse?.data || []);
  ...
  const loadTransfers = useCallback(async () => {
    ...
    try {
      ...
      const normalizedOutDest = outDestSafeId
        ? String(outDestSafeId).trim()
        : "";
      const normalizedInDest = inDestSafeId ? String(inDestSafeId).trim() : "";
      const normalizedUserId = selectedUserId
      ...
      const normalizedTransferId =
        explicitTransferId != null && String(explicitTransferId).trim() !== ""
      ...
      const response = await getSafeTransfersPaginated({
        ...
        transferId: Number.isFinite(normalizedTransferId)
      ...
      if (incoming && typeof incoming === "object") {
        const total = Number(
          incoming.total ?? incoming.total_items ?? incoming.count ?? 0,
        );
        const perPage = Number(
          incoming.per_page ?? incoming.limit ?? incoming.perPage ?? limit,
        );
        const currentPage = Number(
          incoming.page ??
            incoming.current_page ??
            incoming.page_number ??
            page,
        );
        const totalPagesCandidate = Number(
          incoming.total_pages ?? incoming.pages ?? incoming.page_count ?? 0,
        );
        const totalPages =
  ...
  const normalizedTransfers = useMemo(() => {
    ...
    return items.map((item) => {
      ...
      const destination =
        item.safe_transactions_counterpart_safe_id != null
          ? {
      ...
      const reference =
        item.safe_transactions_reference ?? item.transfer_out_reference ?? null;
      const date =
        item.safe_transactions_date ?? item.transfer_out_date ?? null;
      const userName = item.user_name || item.transfer_in_user_name || null;
      const userId =
        item.safe_transactions_user_id ??
      ...
      const transferOutStatus = (
        item.transfer_out_status ||
        item.safe_transactions_status ||
        ""
      ).toLowerCase();
      ...
      const approvedByName =
        item.transfer_out_approved_by_name ||
        item.transfer_in_approved_by_name ||
        null;
      const approvedDate =
        item.transfer_out_approved_date ||
  ...
  const filteredTransfers = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      const normalizedNumeric = /^\d+$/.test(numericCandidate)
        ? numericCandidate
        : null;
    ...
    if (outDestSafeId) {
      rows = rows.filter(
        (row) => String(row.destination?.id ?? "") === String(outDestSafeId),
      );
    }
    ...
    if (inDestSafeId) {
      rows = rows.filter(
        (row) => String(row.source?.id ?? "") === String(inDestSafeId),
      );
    }
  ...
  const totalPages =
    ...
    Math.max(
      1,
      Math.ceil(
        (paginationMeta.total ?? filteredTransfers.length) / (perPage || 1),
      ),
    );
  ...
  const handleActionComplete = useCallback(
    (newStatus) => {
      ...
      if (statusLower === "approved") {
        setGlobalMessage({
          type: "success",
          message: "تمت الموافقة على التحويل بنجاح.",
        });
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (outDestSafeId) {
      const safeName = allSafes.find(
        (safe) => String(safe.safes_id) === String(outDestSafeId),
      )?.safes_name;
      chips.push({
        key: "outDest",
        label: "صادر إلى",
    ...
    if (inDestSafeId) {
      const safeName = allSafes.find(
        (safe) => String(safe.safes_id) === String(inDestSafeId),
      )?.safes_name;
      chips.push({
        key: "inDest",
        label: "من خزنة",
    ...
    if (selectedUserId) {
      const userName = allUsers.find(
        (user) => String(user.users_id) === String(selectedUserId),
      )?.users_name;
      chips.push({
        key: "user",
        label: "المستخدم",
    ...
    () => [
      {
        ...
        sortable: true,
        sortAccessor: (row) =>
          Number(row.transactionId ?? row.transferOutId ?? row.transferInId) ||
          0,
        render: (row) => {
            ...
            <div className="flex flex-wrap items-center gap-1" dir="ltr">
              {idBadges.map((badge) => (
                ...
                >
                  ...
                  <span className="bg-white/70 text-gray-800 px-1.5 py-0.5 rounded">
                    #{badge.value}
                  </span>
                </span>
              ))}
      ...
      },
      {
        ...
        sortable: false,
        render: () => (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full bg-[#EDE7FF] text-[#7A52C2]">
        ...
        render: (row) => (
          <div className="space-y-1">
            ...
            {row.destination?.type && (
              <span className="text-xs text-gray-500">
                {safeTypeLabel(row.destination.type)}
              </span>
            )}
          </div>
        ),
        ...
        render: (row) => {
          ...
          return (
            <div className="flex flex-col items-center gap-1 text-xs">
              <span
                className={`inline-flex items-center px-2.5 py-1 rounded-full font-semibold ${meta.className}`}
              >
                {meta.label}
              </span>
              {row.status !== "pending" && row.approvedByName && (
                <span className="text-[10px] text-gray-400">
                  بواسطة {row.approvedByName}
                </span>
              )}
