...
export default function SafeReportsTab() {
  ...
  const loadSafes = useCallback(async () => {
    ...
    try {
      ...
      setSafes(Array.isArray(response) ? response : response?.safes || response?.data || []);
    } catch (e) {
    ...
    <div className="p-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
        <h3 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <ChartBarIcon className="h-8 w-8 text-[#8B5FD6]" />
          تقارير الخزائن
        </h3>
        <div className="flex-none">
          <button className="bg-[#8B5FD6] hover:bg-[#7A52C2] text-white font-bold py-2 px-4 rounded-md shadow-md flex items-center gap-2">
        ...
        <div className="bg-gradient-to-r from-blue-100 to-blue-50 p-6 rounded-xl border border-[#C4A8F0]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-[#2D1B69]">0.00 {symbol}</p>
            </div>
            <ArrowTrendingUpIcon className="h-12 w-12 text-[#8B5FD6]" />
          </div>
        </div>
