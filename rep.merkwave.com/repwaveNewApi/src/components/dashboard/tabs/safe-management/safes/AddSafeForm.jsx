...
const AddSafeForm = ({ onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const safeData = {
        safes_name: formData.name.trim(),
        safes_description: formData.description.trim(),
        safes_type: formData.type,
        safes_rep_user_id:
          formData.type === "rep" || formData.type === "store_keeper"
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#8B5FD6] focus:border-[#8B5FD6] outline-none resize-none"
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                    {SAFE_COLORS.map((color) => (
                      <button
                        ...
                        className={`relative p-3 rounded-lg border-2 transition-all ${
                          formData.color === color.value
                            ? `${color.borderClass} ring-2 ring-offset-2 ring-[#8B5FD6]`
                      ...
                      >
                        ...
                        {formData.color === color.value && (
                          <div className="absolute -top-1 -right-1 bg-[#8B5FD6] text-white rounded-full w-5 h-5 flex items-center justify-center">
                            <svg
                              className="w-3 h-3"
                              fill="currentColor"
                              viewBox="0 0 20 20"
