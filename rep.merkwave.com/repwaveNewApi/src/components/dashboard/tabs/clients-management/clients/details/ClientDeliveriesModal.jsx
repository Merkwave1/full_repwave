...
import {
  getAppSalesDeliveries,
  getSalesDeliveryDetails,
} from "../../../../../../apis/sales_deliveries.js";
...
export default function ClientDeliveriesModal({ client, open, onClose }) {
  ...
  const handlePrintUnified = async (deliveryArg) => {
    ...
    const clientName =
      delivery.clients_company_name || delivery.client_name || "غير محدد";
    const items =
      details && details.items ? details.items : delivery.items || [];
    const status =
      delivery.sales_deliveries_delivery_status ||
      delivery.delivery_status ||
      delivery.sales_deliveries_status ||
      "";
    const statusArabic =
      status === "Delivered"
        ? "تم التسليم"
            ...
            <tbody>
              ${
                ...
                <td>${it.sales_delivery_items_notes || "-"}</td>
                      ...
                      .join("")
                  : `<tr><td colspan="6" style="text-align:center;">لا توجد عناصر</td></tr>`
              }
            </tbody>
          </table>
          <div class="footer">
  ...
  const handleShowDetails = (delivery) => {
    ...
    fetchDeliveryDetails(delivery).then((d) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError("");
      try {
        const resp = await getAppSalesDeliveries();
        const arr = Array.isArray(resp?.sales_deliveries)
          ? resp.sales_deliveries
          : Array.isArray(resp)
            ? resp
            : resp?.data?.sales_deliveries || resp?.data || [];
        const filtered = Array.isArray(arr)
          ? arr.filter(
              (d) =>
                (d.delivery_client_id ||
                  d.sales_deliveries_client_id ||
                  d.clients_id ||
                  d.client_id ||
                  d.sales_orders_client_id) == client.clients_id,
            )
          : [];
        if (!cancelled) setRows(filtered);
      } catch (e) {
        if (!cancelled) {
          setError(e.message || "فشل تحميل التسليمات");
          setRows([]);
        }
      } finally {
  ...
  const applyFilters = (arr) =>
    arr.filter((r) => {
      ...
      const dateVal =
        r.delivery_date ||
        r.sales_deliveries_delivery_date ||
        r.sales_deliveries_date ||
        r.created_at;
      if (dateFrom && dateVal && new Date(dateVal) < new Date(dateFrom))
        return false;
      if (dateTo && dateVal && new Date(dateVal) > new Date(dateTo))
        return false;
      if (status) {
        const st = (
          r.delivery_status ||
          r.sales_deliveries_delivery_status ||
          r.sales_deliveries_status ||
          ""
        ).toLowerCase();
  ...
  return (
    <>
      ...
      >
        ...
        {loading && (
          <div className="text-[#8B5FD6] font-semibold">تحميل...</div>
        )}
                ...
                <table className="min-w-full text-[11px]">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <Th>الحالة</Th>
                      <Th>ملاحظات</Th>
                      <Th className="no-print">عرض</Th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {filtered.map((r) => (
                      <tr
                        key={r.delivery_id || r.sales_deliveries_id}
                      ...
                      >
                        ...
                        <Td
                          ...
                          title={
                            r.delivery_notes ||
                            r.sales_deliveries_delivery_notes ||
                            r.sales_deliveries_notes
                          }
                        >
                          {r.delivery_notes ||
                            r.sales_deliveries_delivery_notes ||
                        ...
                        <Td>
                          <button
                            ...
                            className="no-print text-[#8B5FD6] hover:underline text-[10px] font-semibold"
                          >
                            تفاصيل
                          </button>
                        </Td>
      ...
      {selected && loadingDetails && !selectedDetails && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center pointer-events-none">
          <div className="bg-white/80 backdrop-blur px-4 py-2 rounded text-xs font-semibold text-[#7A52C2] shadow">
  ...
  return (
    <div className="flex flex-wrap gap-2 text-[11px] items-end">
      <div className="flex flex-col">
        <label className="mb-0.5">من</label>
        <input
          type="date"
      ...
      <div className="flex flex-col">
        <label className="mb-0.5">إلى</label>
        <input
          type="date"
      ...
      <div className="flex flex-col">
        <label className="mb-0.5">حالة</label>
        <input
      ...
      <div className="flex flex-col flex-1 min-w-[160px]">
        <label className="mb-0.5">بحث</label>
        <input
          placeholder="بحث عام"
...
const Th = ({ children }) => (
  <th className="px-2 py-1 text-right font-bold text-gray-700">{children}</th>
);
...
async function fetchDeliveryDetails(delivery) {
  try {
    const details = await getSalesDeliveryDetails(
      delivery.delivery_id || delivery.sales_deliveries_id || delivery.id,
    );
    return details;
  } catch (e) {
