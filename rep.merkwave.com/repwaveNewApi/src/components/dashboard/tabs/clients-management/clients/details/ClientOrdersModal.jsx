  ...
  return (
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map(o=> (
                    <tr key={o.sales_orders_id} className="hover:bg-gray-50">
                      ...
                      <Td><button onClick={()=>handleShowDetails(o)} className="no-print text-[#8B5FD6] hover:underline text-[10px] font-semibold">تفاصيل</button></Td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    {selected && loadingDetails && !selectedDetails && (
      <div className="fixed inset-0 z-[70] flex items-center justify-center pointer-events-none">
        <div className="bg-white/80 backdrop-blur px-4 py-2 rounded text-xs font-semibold text-[#7A52C2] shadow">جاري تحميل تفاصيل الأمر...</div>
      </div>
    )}
  </>
  );
