...
function ClientDetailsModal({
  ...
  const handleAddInterested = async () => {
    ...
    setInterestsFeedback(null);
    try {
      const result = await addClientInterestedProduct(
        clientId,
        selectedProductId,
      );
      const message =
        typeof result === "string" ? result : "تم إضافة المنتج بنجاح.";
  ...
  const handleRemoveInterested = async (productId) => {
    ...
    setInterestsFeedback(null);
    try {
      const result = await removeClientInterestedProduct(clientId, productId);
      const message =
        typeof result === "string" ? result : "تم إزالة المنتج بنجاح.";
      notify?.({ type: "success", message });
      setInterestsFeedback({ type: "success", text: message });
      await refreshInterestedProducts();
    } catch (err) {
            ...
            <DetailItem icon={<GlobeAltIcon />} label="الموقع الإلكتروني">
              {client.clients_website ? (
                <a
                  ...
                  className="text-[#8B5FD6] hover:underline"
                >
                  {client.clients_website}
                </a>
              ) : (
        ...
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <DetailItem
              icon={<PhoneIcon />}
              label="هاتف 1"
              value={client.clients_contact_phone1}
            />
            <DetailItem
              icon={<PhoneIcon />}
              label="هاتف 2"
              value={client.clients_contact_phone2}
            />
          </div>
        );
      case "address":
            ...
            <div className="bg-white border border-[#EDE7FF] rounded-xl shadow-sm p-3 sm:p-5 md:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-[#8B5FD6] mt-1">
              ...
              <div className="mt-5 flex flex-col sm:flex-row gap-3">
                <select
                  className="flex-1 rounded-lg border border-[#C4A8F0] bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#8B5FD6]"
              ...
              {!productsLoading && availableProductCount === 0 && (
                <p className="mt-3 text-sm text-[#7A52C2]">
        ...
        <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 md:p-6 mb-3 sm:mb-6 flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-5 md:gap-6">
          ...
          <div className="flex-grow text-center sm:text-right min-w-0">
            ...
            <div className="mt-2 sm:mt-4 flex flex-wrap justify-center sm:justify-end gap-1.5 sm:gap-3">
              ...
              <span className="bg-[#EDE7FF] text-[#2D1B69] text-xs sm:text-sm font-semibold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full">
