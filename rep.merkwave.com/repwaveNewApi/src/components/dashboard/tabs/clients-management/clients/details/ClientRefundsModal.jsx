...
export default function ClientRefundsModal({ client, open, onClose }) {
  ...
  return (
    ...
    <SharedDetailModalBase title="تفاصيل الاستردادات" client={client} open={open} onClose={onClose}>
      ...
      {loading && <div className="text-[#8B5FD6] font-semibold">تحميل...</div>}
      ...
      <div className="overflow-x-auto border rounded-lg">
        ...
        <thead className="bg-gray-100"><tr><Th>#</Th><Th>التاريخ</Th><Th>المبلغ</Th><Th>طريقة</Th><Th className="no-print">عرض</Th></tr></thead>
            ...
            <Td><button onClick={()=>handleShowDetails(r)} className="no-print text-[#8B5FD6] hover:underline text-[10px] font-semibold">تفاصيل</button></Td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  {selected && (
    ...
    {selected && loadingDetails && !selectedDetails && (
      <div className="fixed inset-0 z-[70] flex items-center justify-center pointer-events-none">
        <div className="bg-white/80 backdrop-blur px-4 py-2 rounded text-xs font-semibold text-[#7A52C2] shadow">جاري تحميل تفاصيل الاسترداد...</div>
      </div>
    )}
    </>
  );
