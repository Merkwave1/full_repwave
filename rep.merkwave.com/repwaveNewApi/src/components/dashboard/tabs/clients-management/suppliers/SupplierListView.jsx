      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="bg-white rounded-xl shadow border border-gray-200 p-12 text-center animate-fadeIn">
          ...
          <p className="text-gray-500 text-sm mt-2">
            جرب البحث بكلمات مختلفة أو أضف موردًا جديدًا
          </p>
        </div>
      )}
          ...
          <div className="bg-gradient-to-r from-[#f5f3ff] to-[#E8DFFF] px-6 py-4 border-b border-gray-300">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-800">
                إجمالي الموردين:
                <span className="font-bold text-[#8B5FD6] ml-1">
                  {filtered.length}
                </span>
              </div>
              {searchTerm && (
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <SortableHeader
                    title="الشخص المسؤول"
                    sortKey="supplier_contact_person"
                    className="min-w-[150px] border-r border-gray-200"
                  />
                  <SortableHeader
                    title="الهاتف"
                    sortKey="supplier_phone"
                    className="min-w-[120px] border-r border-gray-200"
                  />
                  <SortableHeader
                    title="البريد الإلكتروني"
                    sortKey="supplier_email"
                    className="min-w-[200px] border-r border-gray-200"
                  />
                  <SortableHeader
                    title="الرصيد"
                    sortKey="supplier_balance"
                    className="min-w-[120px] border-r border-gray-200"
                  />
                  <th className="px-4 py-3 text-right text-xs font-bold text-gray-700 uppercase min-w-[250px] border-r border-gray-200">
                    العنوان
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-bold text-gray-700 uppercase min-w-[200px] border-r border-gray-200">
                    ملاحظات
                  </th>
                  <th className="w-32 px-4 py-3 text-center text-xs font-bold text-gray-700 uppercase">
                    إجراءات
                  </th>
                </tr>
              </thead>
                ...
                {sortedSuppliers.map((supplier, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-900 font-semibold border-r border-gray-200">
                      <div
                        className={`text-sm ${parseFloat(supplier.supplier_balance || 0) >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
