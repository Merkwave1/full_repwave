...
export default function AddPurchaseReturnForm({ onAdd, onCancel, suppliers }) {
  ...
  const handleSubmit = (e) => {
    ...
    const discountNote = computedDiscount
      ...
      : "";
    onAdd({
      supplier_id: parseInt(formData.purchase_return_supplier_id) || null,
      ...
      items: itemsToSubmit.map((it) => ({
        purchase_order_item_id: it.original_item_id
          ? parseInt(it.original_item_id)
          : null,
        quantity: parseInt(it.quantity_returned) || 0,
        unit_cost: parseFloat(it.unit_cost) || 0,
        notes: it.return_reason || null,
      })),
    });
  };
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <div className="flex items-center justify-between px-4 sm:px-8 py-4 sm:py-5 bg-gradient-to-l from-[#1A0F35] to-[#7C1D1D] text-white">
        <div>
            ...
            {!formData.purchase_return_purchase_order_id ? (
              ...
              formData.return_items.length > 0 && (
                <div className="flex flex-wrap gap-2 text-[11px]">
                  ...
                  <span className="bg-[#EDE7FF] text-[#7A52C2] px-2 py-0.5 rounded-full font-medium">
                    في المخزون:{" "}
                    {
                      formData.return_items.filter(
