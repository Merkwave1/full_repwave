...
export default function PurchaseReturnsTab() {
  ...
  const handleEditReturn = useCallback((returnData) => {
    setSelectedReturn(returnData);
    setCurrentView("edit");
  }, []);
    ...
    return (
      <>
        <CustomPageHeader
          ...
          icon={<ArrowUturnLeftIcon className="w-6 h-6 text-[#1A0F35]" />}
          ...
          actionButton={
            <button
              ...
              className="px-3 py-2 bg-[#1A0F35] text-[#C4A8F0] hover:bg-[#374151] hover:scale-110 rounded-md font-semibold"
            >
              إضافة مرتجع شراء
            </button>
          }
