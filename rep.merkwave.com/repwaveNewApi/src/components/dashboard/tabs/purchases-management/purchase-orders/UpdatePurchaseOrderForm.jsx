...
function UpdatePurchaseOrderForm({
  ...
  const [formData, setFormData] = useState(() => {
    return {
      ...
      purchase_orders_order_date: order.purchase_orders_order_date
        ? format(
            new Date(order.purchase_orders_order_date),
            "yyyy-MM-dd'T'HH:mm",
          )
        : "",
      ...
      order_discount:
        order.purchase_orders_order_discount ||
        order.purchase_orders_order_discount ||
        "",
  ...
  const getProductDetailsByVariantId = useCallback((variantId, allProducts) => {
    if (!Array.isArray(allProducts))
      return { productId: "", productBaseUnitId: null };
    const product = allProducts.find(
      (p) =>
        Array.isArray(p.variants) &&
        p.variants.some((v) => v.variant_id === variantId),
    );
    return {
  ...
  useEffect(() => {
    if (
      ...
      const updatedItems = order.items.map((item) => {
        const { productId, productBaseUnitId } = getProductDetailsByVariantId(
        ...
        return {
          ...
          variant_id:
            item.purchase_order_items_variant_id != null
              ? String(item.purchase_order_items_variant_id)
              : "",
          quantity_ordered:
            item.purchase_order_items_quantity_ordered != null
              ? String(item.purchase_order_items_quantity_ordered)
              : "",
          unit_cost:
            item.purchase_order_items_unit_cost != null
              ? String(item.purchase_order_items_unit_cost)
              : "",
          packaging_type_id:
            item.purchase_order_items_packaging_type_id != null
              ? String(item.purchase_order_items_packaging_type_id)
              : "",
  ...
  useEffect(() => {
    ...
    if (
      ...
      setFormData((prevData) => ({
        ...prevData,
        purchase_orders_warehouse_id: String(
          order.purchase_orders_warehouse_id,
        ),
      }));
    }
  ...
  const handleItemVariantSelect = (index, variantId) => {
    ...
    const selectedVariant = allVariantsOptions.find(
      (v) => v.value === variantId,
    );
  ...
  const handleSubmit = (e) => {
    ...
    if (orderDateForBackend && orderDateForBackend.includes("T")) {
      ...
      if (orderDateForBackend.length === 16)
        orderDateForBackend = orderDateForBackend + ":00";
    }
    ...
    onUpdate({
      ...
      warehouse_id: formData.purchase_orders_warehouse_id
        ...
        : null,
      order_date: orderDateForBackend || null,
      notes: ((formData.purchase_orders_notes || "") + discountNote) || null,
      status: formData.purchase_orders_status || "Draft",
  ...
  return (
    <div
      className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-md max-w-4xl mx-auto"
      dir="rtl"
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            >
              ...
              {Array.isArray(warehouses) &&
                warehouses.map((warehouse) => (
                  ...
                  >
                    {" "}
                    {/* FIX: Convert warehouse.warehouse_id to string */}
        ...
        <div className="border-t border-gray-200 pt-6 mt-6">
          ...
          {formData.purchase_order_items.length === 0 && (
            <p className="text-center text-gray-600">
              لا توجد عناصر في أمر الشراء. أضف عنصرًا واحدًا على الأقل.
            </p>
          )}
          ...
          {formData.purchase_order_items.map((item, index) => {
            ...
            return (
              <div
                key={index}
                ref={
                  index === formData.purchase_order_items.length - 1
                    ? lastItemRef
                    : null
                }
                className="bg-gray-50 p-3 rounded-md shadow-sm mb-3 border border-gray-200"
              >
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 items-start">
                  {/* Variant */}
                  ...
                  <div className="text-center">
                    ...
                    <div className="px-1 py-1.5 bg-gray-100 border border-gray-300 rounded-md text-xs font-semibold">
                      {formatMoney(total)}
                    </div>
                  </div>
                  {/* Remove */}
        ...
        <div className="mt-10">
          <div className="max-w-4xl mx-auto bg-gray-50 p-3 sm:p-4 md:p-6 rounded-lg border border-gray-200 space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-stretch text-center">
              <div className="min-h-24 flex flex-col justify-center items-center">
                ...
                <div className="w-full px-4 py-3 bg-white border border-gray-300 rounded-md text-lg font-semibold text-gray-800">
              ...
              <div className="min-h-24 flex flex-col justify-center items-center">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full px-2 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#8B5FD6] focus:border-[#8B5FD6] text-sm text-center"
                >
                  {order.purchase_orders_status === "Draft" ? (
                    ...
                    <option value={order.purchase_orders_status}>
                ...
                {order.purchase_orders_status !== "Draft" && (
                  <p className="text-xs text-gray-500 mt-1">
                    لا يمكن تغيير الحالة إلا من المسودة (Draft) إلى مطلوب
                    (Ordered) أو ملغي (Cancelled)
                  </p>
                )}
              </div>
              <div className="min-h-24 flex flex-col justify-center items-center">
            ...
            <div className="flex items-center justify-between text-lg font-bold text-gray-800">
              ...
              <span className="text-green-700">
                {formatMoney(orderTotals.finalTotal || 0)}{" "}
