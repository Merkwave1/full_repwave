...
export default function SupplierPaymentsTab() {
  ...
  const loadContextData = useCallback(async () => {
    ...
    const fetchAndCache = async (keys, fetchFn, setter) => {
      const raw = keys.reduce((found, k) => found || localStorage.getItem(k), null);
      const cached = parseList(raw);
      if (cached.length > 0) {
        setter(cached);
        return;
      }
    ...
    try {
      await Promise.all([
        fetchAndCache(
          ["appPaymentMethods", "app_payment_methods"],
          getPaymentMethods,
          setPaymentMethods,
        ),
        fetchAndCache(
        ...
        fetchAndCache(
          ["appSuppliers", "app_suppliers", "suppliers"],
          getAllSuppliers,
          setSuppliers,
        ),
      ]);
  ...
  const tableColumns = [
    {
      ...
      render: (r) => (
        <span className="bg-[#EDE7FF] text-[#1A0F35] text-xs px-2.5 py-1 rounded-full font-semibold">
    ...
    return (
      <>
        <CustomPageHeader
          ...
          icon={<BanknotesIcon className="w-6 h-6 text-[#1A0F35]" />}
          ...
          actionButton={
            <button
              ...
              className="px-3 py-2 flex items-center gap-2 bg-[#1A0F35] text-[#C4A8F0] hover:bg-[#374151] hover:scale-110 rounded-md font-semibold"
            >
