    ...
    () => [
      {
        ...
        title: "ID",
        sortable: true,
        sortAccessor: (it) => Number(it.sales_orders_id || it.id || 0),
        headerAlign: "center",
        align: "center",
        render: (item) => (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#EDE7FF] text-[#1A0F35]">
            {item.sales_orders_id || item.id || "—"}
          </span>
        ),
        className: "w-20",
      },
      ...(odooEnabled
        ? [
            {
              key: "sales_orders_odoo_invoice_id",
              title: "ODOO ID",
              sortable: true,
              headerAlign: "center",
              align: "center",
              ...
              render: (it) => (
                <span className="font-mono text-xs text-gray-600">
                  {it.sales_orders_odoo_invoice_id || "-"}
                </span>
              ),
            },
          ]
      ...
      },
      {
        ...
        render: (it) => (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#EDE7FF] text-[#2D1B69]">
            {it.items_count ||
              it.total_items ||
              (Array.isArray(it.items) ? it.items.length : 0) ||
              0}{" "}
      ...
      },
      {
        ...
        sortable: true,
        sortAccessor: (it) =>
          new Date(it.sales_orders_order_date || it.order_date).getTime() || 0,
        className: "min-w-[100px]",
        render: (it) => (
          <div className="text-sm text-gray-700">
            {it.sales_orders_order_date || it.order_date
              ? new Date(
                  it.sales_orders_order_date || it.order_date,
      ...
      {
        key: "sales_orders_status",
        title: "الحالة",
        sortable: true,
        className: "min-w-[100px]",
        render: (it) => (
          <span
            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(it.sales_orders_status || it.status)}`}
          >
            {getStatusDisplay(it.sales_orders_status || it.status)}
          </span>
        ),
      },
      {
        key: "sales_orders_delivery_status",
        title: "حالة التسليم",
        sortable: true,
        className: "min-w-[120px]",
        render: (it) => (
          <span
            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDeliveryStatusColor(it.sales_orders_delivery_status || it.delivery_status)}`}
          >
            {getDeliveryStatusDisplay(
              it.sales_orders_delivery_status || it.delivery_status,
            )}
          </span>
        ),
      },
      {
        ...
        sortable: true,
        sortAccessor: (it) =>
          Number(it.sales_orders_total_amount || it.total_amount || 0),
        className: "min-w-[120px]",
        render: (it) => (
          <div className="text-green-600 font-semibold">
            {formatCurrency(
              parseFloat(it.sales_orders_total_amount || it.total_amount || 0),
            )}
          </div>
        ),
      },
      {
        ...
        render: (it) => {
          ...
          const deliveryStatus =
            it.sales_orders_delivery_status || it.delivery_status;
          const canDeliver =
            orderStatus === "Invoiced" && deliveryStatus !== "Delivered";
          return (
            <div className="flex items-center justify-center space-x-reverse space-x-2">
              ...
              <button
                onClick={() => onEdit(it)}
                disabled={
                  orderStatus === "Cancelled" || orderStatus === "Invoiced"
                }
      ...
      orderStatus === "Cancelled" || orderStatus === "Invoiced"
        ...
        : `text-emerald-700 bg-emerald-100
           ...
           hover:scale-110`
                ...
                title={
                  orderStatus === "Cancelled" || orderStatus === "Invoiced"
                    ? "لا يمكن تعديل أمر ملغي أو مفوتر"
                    : "تعديل"
                }
              >
