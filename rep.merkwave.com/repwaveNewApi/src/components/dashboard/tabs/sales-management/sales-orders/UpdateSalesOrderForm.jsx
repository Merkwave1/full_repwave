...
const normalizePackagingEntry = (entry) => {
  const packagingId = normalizePackagingTypeId(
    entry?.packaging_type_id ?? entry?.packaging_types_id ?? entry?.id ?? entry,
  );

  return {
    id: packagingId,
    available_quantity:
      parseFloat(entry?.available_quantity ?? entry?.inventory_quantity ?? 0) ||
      0,
    unit_price: entry?.unit_price ?? null,
    isAvailable: Boolean(
      entry?.isAvailable &&
      (parseFloat(entry?.available_quantity ?? 0) || 0) > 0,
    ),
    packaging_conversion_factor:
      entry?.packaging_conversion_factor ??
      entry?.conversion_factor ??
      entry?.packaging_default_conversion_factor ??
      1,
...
const arePackagingInventoriesEqual = (listA, listB) => {
  ...
  for (let i = 0; i < normalizedA.length; i += 1) {
    ...
    if (
      !numbersAreEqual(
        a.packaging_conversion_factor,
        b.packaging_conversion_factor,
      )
    ) {
      return false;
    }
...
const collectPreferredPackagingTypes = (
  ...
  return flattened
    .map((entry) => {
      ...
      const packagingMeta = allPackagingTypes.find(
        (pt) =>
          normalizePackagingTypeId(pt.packaging_types_id) === packagingTypeId,
      );
      ...
      return {
        ...
        packaging_type_name:
          entry?.packaging_type_name ||
          entry?.packaging_types_name ||
          packagingMeta?.packaging_types_name,
...
export default function UpdateSalesOrderForm({
  ...
  const initialRepId =
    ...
    order?.representatives_id ||
    "";
  const normalizedRepId =
    initialRepId === "" || initialRepId === null || initialRepId === undefined
      ? ""
      : Number(initialRepId);
  const initialOrderDateRaw =
    order?.sales_orders_order_date ||
    order?.sales_order_date ||
    order?.order_date;
  const normalizedOrderDate =
    toLocalDateTimeInputValue(initialOrderDateRaw) ||
  ...
  const [formData, setFormData] = useState({
    sales_orders_client_id:
      order?.sales_orders_client_id || order?.client_id || "",
    sales_orders_warehouse_id:
      order?.sales_orders_warehouse_id || order?.warehouse_id || "",
    ...
    sales_orders_expected_delivery_date:
      ...
      order?.expected_delivery_date
        ...
        : "",
    sales_orders_discount_amount: parseFloat(
      order?.sales_orders_discount_amount || order?.discount_amount || 0,
    ),
    sales_orders_tax_amount: parseFloat(
      order?.sales_orders_tax_amount || order?.tax_amount || 0,
    ),
  ...
  useEffect(() => {
    if (order) {
      ...
      const transformedItems = orderItems.map((item) => ({
        ...
        products_id:
          item.sales_order_items_product_id ||
          item.products_id ||
          item.product_id ||
          "",
        packaging_type_id:
        ...
        item_notes:
          item.sales_order_items_notes || item.item_notes || item.notes || "",
        ...
        base_unit_price:
          item.base_unit_price || item.sales_order_items_unit_price || null,
        variant_unit_price: item.variant_unit_price || null,
        original_unit_price:
          item.original_unit_price ||
          item.sales_order_items_original_unit_price ||
          item.sales_order_items_unit_price ||
          null,
        original_tax_rate:
          item.original_tax_rate ||
          item.sales_order_items_original_tax_rate ||
      ...
      setFormData({
        ...
        sales_orders_status:
          order.sales_orders_status || order.status || "Draft",
        sales_orders_delivery_status:
          order.sales_orders_delivery_status ||
          order.delivery_status ||
          "Not_Delivered",
  ...
  const availableProducts = useMemo(() => {
    ...
    const safeInventory = Array.isArray(availableInventory)
      ? availableInventory
      : [];
    ...
    safeInventory.forEach((inv) => {
      const variantId = normalizePackagingTypeId(
        inv.variant_id || inv.product_variant_id,
      );
      if (!variantId) {
        return;
      }
    ...
    const buildPackagingInventory = (variant = {}, product = {}) => {
      ...
      variantInventory.forEach((inv) => {
        ...
        const packagingMeta = allPackagingTypes.find(
          (pt) =>
            normalizePackagingTypeId(pt.packaging_types_id) ===
            normalizePackagingTypeId(inv.packaging_type_id),
        );
        ...
        const quantity =
          parseFloat(inv.inventory_quantity || inv.available_quantity || 0) ||
          0;
        ...
        if (packagingMap.has(packagingKey)) {
          ...
          if (
            (!existing.packaging_conversion_factor ||
              existing.packaging_conversion_factor === 1) &&
            conversionFactor
          ) {
      ...
      if (product.packaging_variant_map) {
        product.packaging_variant_map
          .filter(
            (pvm) =>
              normalizePackagingTypeId(pvm.variant_id) ===
              normalizePackagingTypeId(variant.variant_id),
          )
          .forEach((pvm) => {
            ...
            const packagingMeta = allPackagingTypes.find(
              (pt) =>
                normalizePackagingTypeId(pt.packaging_types_id) ===
                normalizePackagingTypeId(pvm.packaging_type_id),
            );
            const conversionFactor = extractConversionFactor({
              ...pvm,
              ...packagingMeta,
            ...
            if (packagingMap.has(packagingKey)) {
              ...
              packagingMap.set(packagingKey, {
                ...
                preferred_order:
                  pvm?.preferred_order ?? pvm?.order ?? pvm?.priority ?? null,
              });
            }
          });
      }
      ...
      preferredPackaging.forEach((pref) => {
        ...
        const packagingName =
          pref.packaging_type_name ||
          pref.packaging_types_name ||
          `نوع تعبئة ${normalizedId}`;
      ...
      return Array.from(packagingMap.values())
        .map((pkg) => ({
          ...
          isAvailable: Boolean(
            pkg.isAvailable && parseFloat(pkg.available_quantity || 0) > 0,
          ),
        }))
        .sort((a, b) => {
    ...
    safeInventory.forEach((inv) => {
      ...
      const fallbackVariant = {
        variant_id: variantId,
        variant_name:
          inv.variant_name || inv.product_variant_name || inv.product_name,
      ...
      const packagingInventory = buildPackagingInventory(
        fallbackVariant,
        fallbackProduct,
      );
      const isInStock = packagingInventory.some((pkg) => pkg.isAvailable);
      const baseName =
        fallbackVariant.variant_name ||
        fallbackProduct.products_name ||
        "منتج غير محدد";
    ...
    (formData.sales_order_items || []).forEach((item) => {
      ...
      if (!productMap.has(variantId)) {
        productMap.set(variantId, {
          ...
          variant_id: variantId,
          products_unit_of_measure_id: item.products_unit_of_measure_id || null,
          packaging_inventory: Array.isArray(item.packaging_inventory)
            ? item.packaging_inventory
            : [],
          variant_unit_price: item.unit_cost || null,
          variant_cost_price: null,
          isInStock: Boolean(
            item.available_quantity && item.available_quantity > 0,
          ),
        ...
        Array.isArray(item.packaging_inventory) &&
        item.packaging_inventory.length > 0
      ) {
        const entry = productMap.get(variantId);
        const existingIds = new Set(
          entry.packaging_inventory.map((pkg) => pkg.packaging_type_id),
        );
    ...
    const loadInventory = async () => {
      ...
      if (cachedData) {
        try {
          ...
          console.warn(
            "Failed to parse cached inventory, will fetch fresh data",
          );
        }
      }
      ...
      try {
        ...
        // api.get already unwraps PagedResult → array
    ...
    setFormData((prevData) => {
      ...
      const nextItems = prevData.sales_order_items.map((item) => {
        ...
        const productData = availableProducts.find(
          (p) => p.value === item.variant_id,
        );
        if (!productData) {
          return item;
        }

        const newPackagingInventory = Array.isArray(
        ...
        const needsProductMetaUpdate =
          ...
          item.products_unit_of_measure_id !==
            productData.products_unit_of_measure_id;
        ...
        const normalizedPackagingId = normalizePackagingTypeId(
          item.packaging_type_id,
        );
        if (normalizedPackagingId !== null && newPackagingInventory.length) {
          const selectedPackaging = newPackagingInventory.find(
            (pkg) =>
              normalizePackagingTypeId(pkg.packaging_type_id) ===
              normalizedPackagingId,
          );

          if (selectedPackaging) {
            const parsedAvailable =
              parseFloat(selectedPackaging.available_quantity ?? 0) || 0;
            if (
              !numbersAreEqual(parsedAvailable, item.available_quantity ?? 0)
            ) {
            ...
            const derivedAvailability = Boolean(
              selectedPackaging.isAvailable &&
              (parseFloat(selectedPackaging.available_quantity ?? 0) || 0) > 0,
            );
            ...
            const derivedConversion =
              selectedPackaging.packaging_conversion_factor ??
              item.packaging_conversion_factor ??
              1;
            if (
              !numbersAreEqual(
                derivedConversion,
                item.packaging_conversion_factor ?? 1,
              )
            ) {
        ...
        if (needsProductMetaUpdate) {
          ...
          updatedItem.products_unit_of_measure_id =
            productData.products_unit_of_measure_id;
        }
        ...
        if (shouldUpdateConversion) {
          updatedItem.packaging_conversion_factor =
            nextPackagingConversion ?? 1;
        }
  ...
  const calculateItemTotals = (item) => {
    const quantity =
      parseFloat(item.quantity_ordered || item.sales_order_items_quantity) || 0;
    const unitCost =
      parseFloat(item.unit_cost || item.sales_order_items_unit_price) || 0;
    const discountAmount =
      parseFloat(
        item.discount_amount || item.sales_order_items_discount_amount,
      ) || 0;
    const taxRate =
      parseFloat(item.tax_rate || item.sales_order_items_tax_rate) || 0;
    ...
    const taxApplies =
      Boolean(item.has_tax || item.sales_order_items_has_tax) || taxRate > 0;
      ...
      if (name === "packaging_type_id") {
        if (fieldValue && newItems[index].packaging_inventory) {
          ...
          const selectedPackaging = newItems[index].packaging_inventory.find(
            (pkg) =>
              normalizePackagingTypeId(pkg.packaging_type_id) === normalizedId,
          );

          if (selectedPackaging) {
            newItems[index].available_quantity =
              selectedPackaging.available_quantity;
            newItems[index].isPackagingAvailable =
              selectedPackaging.isAvailable;
            newItems[index].packaging_conversion_factor =
              selectedPackaging.packaging_conversion_factor || 1;

            if (
              (newItems[index].unit_cost === "" ||
                newItems[index].unit_cost === null ||
                newItems[index].unit_cost === undefined) &&
              selectedPackaging.unit_price
            ) {
  ...
  const handleItemVariantSelect = (index, selectedVariantValue) => {
    ...
    if (selectedVariantData) {
      ...
      setFormData((prevData) => {
        ...
        const hasExistingPackaging =
          currentPackagingId &&
          newPackagingInventory.some(
            (pkg) =>
              normalizePackagingTypeId(pkg.packaging_type_id) ===
              normalizePackagingTypeId(currentPackagingId),
          );

        const effectivePackagingId = hasExistingPackaging
          ? currentPackagingId
          : "";
        const selectedPackaging = hasExistingPackaging
          ? newPackagingInventory.find(
              (pkg) =>
                normalizePackagingTypeId(pkg.packaging_type_id) ===
                normalizePackagingTypeId(effectivePackagingId),
            )
          : null;
        ...
        newItems[index] = {
          ...
          available_quantity: selectedPackaging
            ? selectedPackaging.available_quantity
            : 0,
          isPackagingAvailable: selectedPackaging
            ? selectedPackaging.isAvailable
            : undefined,
          packaging_conversion_factor: selectedPackaging
            ? selectedPackaging.packaging_conversion_factor || 1
            : 1,
          quantity_ordered: currentItem.quantity_ordered ?? "",
          base_unit_price:
            currentItem.base_unit_price ??
            selectedVariantData.variant_unit_price ??
            null,
  ...
  const handleSubmit = (e) => {
    ...
    const itemsToSubmit = formData.sales_order_items.map((item) => ({
      ...
      sales_order_items_quantity:
        item.quantity_ordered || item.sales_order_items_quantity,
      sales_order_items_unit_price:
        item.unit_cost || item.sales_order_items_unit_price,
      sales_order_items_subtotal: (
        parseFloat(
          item.quantity_ordered || item.sales_order_items_quantity || 0,
        ) * parseFloat(item.unit_cost || item.sales_order_items_unit_price || 0)
      ).toFixed(2),
      ...
      sales_order_items_tax_rate:
        item.tax_rate || item.sales_order_items_tax_rate || 0,
      sales_order_items_has_tax:
        item.has_tax || item.sales_order_items_has_tax || false,
      sales_order_items_total_price: calculateItemTotals(item).total,
      sales_order_items_notes:
        item.item_notes || item.sales_order_items_notes || null,
  ...
  const orderTotals = useMemo(() => {
    const subtotal = formData.sales_order_items.reduce((total, item) => {
      const quantity =
        parseFloat(item.quantity_ordered || item.sales_order_items_quantity) ||
        0;
      const unitCost =
        parseFloat(item.unit_cost || item.sales_order_items_unit_price) || 0;
    ...
    const itemsTaxTotal = formData.sales_order_items.reduce((total, item) => {
      return (
        total +
        (parseFloat(item.tax_amount || item.sales_order_items_tax_amount) || 0)
      );
    }, 0);
  ...
  if (
    ...
    return (
      <div
        className="bg-white p-4 sm:p-6 md:p-8 rounded-lg shadow-md max-w-xl mx-auto text-center"
        dir="rtl"
      >
        <ExclamationTriangleIcon className="mx-auto h-12 w-12 text-yellow-500" />
        <h3 className="mt-4 text-lg sm:text-2xl font-bold text-gray-800">
          لا توجد مخازن متاحة
        </h3>
        <p className="mt-2 text-gray-600">
          يجب عليك أولاً إضافة مخزن قبل تعديل أمر البيع.
        </p>
  ...
  return (
    <div
      className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-md max-w-6xl mx-auto"
      dir="rtl"
    >
      ...
      <form className="space-y-6">
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            {inventoryLoading && (
              <div className="mb-4 p-3 bg-[#f5f3ff] border border-[#C4A8F0] rounded-md">
                <p className="text-sm text-[#7A52C2]">
                  جاري تحميل المنتجات المتاحة...
                </p>
              </div>
            )}
            ...
            {!inventoryLoading &&
              ...
              formData.sales_orders_warehouse_id && (
                <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <p className="text-sm text-yellow-700">
                    لا توجد منتجات متاحة في المستودع المحدد. تأكد من وجود مخزون
                    في المستودع.
                  </p>
                </div>
              )}
            ...
            {formData.sales_order_items.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm mt-1">
                  انقر على "إضافة عنصر" لإضافة منتج
                </p>
              </div>
            ) : (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 items-end">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            required
                            className="block w-full px-1 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#8B5FD6] focus:border-[#8B5FD6] text-xs"
                            dir="rtl"
                          >
                            <option value="">اختر تعبئة</option>
                            {getUniquePackagingTypes(
                              item.packaging_inventory,
                            ).map((pkg) => (
                              <option
                        ...
                        <div>
                          ...
                          {(item.original_unit_price ||
                            ...
                            <p className="mt-0.5 text-[10px] text-gray-500">
                              السعر الأصلي:{" "}
                              {formatCurrency(Number(item.original_unit_price))}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {(item.original_tax_rate ||
                            ...
                            <p className="mt-0.5 text-[10px] text-gray-500">
                              الضريبة الأصلية:{" "}
                              {parseFloat(item.original_tax_rate).toFixed(2)}%
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <div className="block w-full px-1 py-1.5 bg-gray-100 border border-gray-300 rounded-md text-xs font-semibold text-gray-900 text-center">
                            {formatCurrency(
                              Number(calculateItemTotals(item).total || 0),
                            )}
                          </div>
                        </div>
                      </div>
                            ...
                            {item.variant_id &&
                              ...
                              item.packaging_inventory.length > 0 && (
                                <p className="text-[#8B5FD6]">
                                  ...
                                  متاح / {item.packaging_inventory.length}{" "}
                                  إجمالي
                                </p>
                              )}
                            {item.variant_id &&
                              (!item.packaging_inventory ||
                                ...
                                <p className="text-red-500">
                                  لا توجد أنواع تعبئة لهذا المنتج
                                </p>
                              )}
                          </div>
                          <div>
                            {item.packaging_type_id &&
                              item.isPackagingAvailable === false && (
                                <p className="text-orange-500">
                                  نوع التعبئة غير متاح في المخزن
                                </p>
                              )}
                            {item.packaging_type_id &&
                              item.isPackagingAvailable === true && (
                                <p className="text-green-600">
                                  نوع التعبئة متاح في المخزن
                                </p>
                              )}
                          </div>
                          <div>
                          ...
                          <div>
                            {Number(
                              ...
                              <p className="text-gray-500">
                                ...
                                {formatCurrency(
                                  Number(
                                    calculateItemTotals(item).discount_amount ||
                                      0,
                                  ),
                                )}
                              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col">
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData({ ...formData, sales_orders_notes: e.target.value })
              }
              className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#8B5FD6] focus:border-[#8B5FD6]"
          ...
          <div className="mt-6 p-4 bg-gray-100 rounded-md shadow-inner">
            <div className="space-y-2">
              ...
              {parseFloat(orderTotals.tax) > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-[#8B5FD6]">
                    إجمالي الضريبة:
                  </span>
                  <span className="text-sm font-semibold text-[#8B5FD6]">
        ...
        <div className="flex justify-end space-x-reverse space-x-3 pt-6 border-t border-gray-200">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#8B5FD6]"
          >
            إلغاء
          </button>
          ...
          {formData.sales_orders_status === "Pending" ? (
            ...
            <>
              ...
              <button
                type="button"
                onClick={() =>
                  setConfirmDialog({ isOpen: true, action: "create_invoice" })
                }
                disabled={loading || formData.sales_order_items.length === 0}
                className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
              >
            ...
            <button
              ...
              disabled={loading || formData.sales_order_items.length === 0}
              className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#8B5FD6] hover:bg-[#7A52C2] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#8B5FD6] disabled:opacity-50"
            >
      ...
      <ConfirmationDialog
        ...
        cancelText="إلغاء"
        danger={
          confirmDialog.action === "reject" || confirmDialog.action === "cancel"
        }
        onConfirm={() => {
          const newStatus =
            confirmDialog.action === "approve"
          ...
          setTimeout(() => {
            ...
            const emptyItems = updatedFormData.sales_order_items.filter(
              (item) => !item.variant_id || !item.packaging_type_id,
            );
            if (emptyItems.length > 0) {
              alert(
            ...
            const itemsToSubmit = updatedFormData.sales_order_items.map(
              (item) => ({
                sales_order_items_variant_id: item.variant_id,
                sales_order_items_packaging_type_id: item.packaging_type_id,
                sales_order_items_quantity:
                ...
                sales_order_items_subtotal: (
                  ...
                  parseFloat(
                    item.unit_cost || item.sales_order_items_unit_price || 0,
                  )
                ).toFixed(2),
                sales_order_items_discount_amount: (
                  (parseFloat(
            ...
            // Calculate totals
            const subtotal = updatedFormData.sales_order_items.reduce(
              (total, item) => {
                const quantity =
                  parseFloat(
                    item.quantity_ordered || item.sales_order_items_quantity,
            ...
            const itemsDiscountTotal = updatedFormData.sales_order_items.reduce(
              (total, item) => {
                return (
                  total +
                  (parseFloat(
            ...
            const orderDiscount =
              parseFloat(updatedFormData.sales_orders_discount_amount) || 0;
            const totalDiscount = itemsDiscountTotal + orderDiscount;
            const afterDiscount = Math.max(0, subtotal - totalDiscount);

            const itemsTaxTotal = updatedFormData.sales_order_items.reduce(
              (total, item) => {
                return (
                  total +
                  (parseFloat(
                    item.tax_amount || item.sales_order_items_tax_amount,
                  ) || 0)
                );
              },
              0,
            ...
            const transformedData = {
              ...
              sales_orders_order_date: resolveOrderDateForSubmission(
                updatedFormData.sales_order_date,
              ),
              sales_orders_status: newStatus,
              sales_orders_delivery_status:
                updatedFormData.sales_orders_delivery_status,
              sales_orders_notes: updatedFormData.sales_orders_notes,
              sales_orders_warehouse_id:
                updatedFormData.sales_orders_warehouse_id,
              sales_orders_representative_id:
                updatedFormData.sales_orders_representative_id,
              representative_id: updatedFormData.sales_orders_representative_id,
              sales_orders_expected_delivery_date:
                updatedFormData.sales_orders_expected_delivery_date || null,
              sales_orders_subtotal: subtotal.toFixed(2),
              sales_orders_discount_amount:
                updatedFormData.sales_orders_discount_amount,
