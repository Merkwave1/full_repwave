...
export default function SalesReturnDetailsModal({
  ...
  const data =
    returnItem &&
    returnItem.data &&
    (returnItem.data.returns_id || Array.isArray(returnItem.data.items))
      ? returnItem.data
      : returnItem;

  const returnNumber =
    data.returns_return_number ||
    data.return_number ||
    data.returns_id ||
    data.id ||
    "-";
  ...
  const clientName =
    data.clients_company_name ||
    data.returns_client_name ||
    data.client_name ||
    clients.find((c) => (c.clients_id || c.id) == clientId)
      ?.clients_company_name ||
    "غير محدد";
  const warehouseId = data.returns_warehouse_id || data.warehouse_id;
  const warehouseName = warehouseId
    ? warehouses.find((w) => (w.warehouses_id || w.id) == warehouseId)
        ?.warehouses_name || warehouses.find((w) => w.id == warehouseId)?.name
    : null;
  ...
  const computeItemTax = (it) => {
    ...
    const unitPrice =
      Number(
        it.return_items_unit_price || it.sales_order_items_unit_price || 0,
      ) || 0;
    const discountTotal =
      Number(
    ...
    const orderedQty =
      Number(it.ordered_quantity || it.sales_order_items_quantity || qty) || 1;
    ...
    const salesTaxRate =
      Number(it.sales_order_items_tax_rate || it.return_items_tax_rate || 0) ||
      0;

    const hasExplicitReturnTax =
      returnTaxRaw !== undefined &&
      returnTaxRaw !== null &&
      String(returnTaxRaw).trim() !== "";
    const explicitReturnTaxValue = hasExplicitReturnTax
      ? Number(returnTaxRaw) || 0
      : null;
  ...
  const sums = (() => {
    ...
    data.items.forEach((it) => {
      ...
      const orderedQty =
        Number(it.ordered_quantity || it.sales_order_items_quantity || qty) ||
        1;
      const itemDiscount = qty > 0 ? (discountTotal / orderedQty) * qty : 0;
  ...
  const itemDiscountSum = Array.isArray(data.items)
    ? data.items.reduce((sum, it) => {
        ...
        const orderedQty =
          Number(it.ordered_quantity || it.sales_order_items_quantity || 1) ||
          1;
  ...
  const totalOrderedQty = Array.isArray(data.items)
    ? data.items.reduce(
        (s, it) =>
          s +
          (Number(it.ordered_quantity || it.sales_order_items_quantity || 0) ||
            0),
        0,
      )
    : 0;
  const totalReturnedQty = Array.isArray(data.items)
    ? data.items.reduce(
        (s, it) =>
          s + (Number(it.return_items_quantity || it.quantity || 0) || 0),
        0,
      )
    : 0;
  const proratedOrderDiscount =
    headerOrderDiscount && totalOrderedQty > 0
  ...
  const computedTotal =
    effectiveSubtotal + effectiveTax - proratedOrderDiscount;
  const showHeaderNote =
    headerReportedTotal > 0 &&
    Math.abs(headerReportedTotal - computedTotal) > 0.01;
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(statusVal)}`}
                    >
                ...
                {salesOrderId && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">
                      رقم أمر البيع المرتبط
                    </label>
                    <p className="mt-1 text-sm text-[#7A52C2] font-medium">
                      {salesOrderId}
                    </p>
                  </div>
                )}
                    ...
                    <p className="mt-1 text-sm text-gray-900">
                      ...
                      {showHeaderNote && (
                        <span className="text-xs text-orange-500 ml-1">
                          (القيمة الأصلية: {formatCurrency(headerReportedTotal)}
                          )
                        </span>
                      )}
                    </p>
                      ...
                      <div className="space-y-3">
                        ...
                        {itemDiscountSum > 0 && (
                          <div className="flex justify-between items-center py-2">
                            ...
                            <span className="font-medium text-gray-900">
                              {formatCurrency(itemDiscountSum)}
                            </span>
                          </div>
                        )}
                        ...
                        {proratedOrderDiscount > 0 && (
                          <div className="flex justify-between items-center py-2">
                            ...
                            <span className="font-medium text-gray-900">
                              {formatCurrency(proratedOrderDiscount)}
                            </span>
                          </div>
                        )}
                        ...
                        {totalDisplayedDiscount > 0 && (
                          <div className="flex justify-between items-center py-2">
                            ...
                            <span className="font-medium text-gray-900">
                              {formatCurrency(totalDisplayedDiscount)}
                            </span>
                          </div>
                        )}
                    ...
                    <table className="min-w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {items.map((it, idx) => {
                          const qty =
                            Number(
                              it.return_items_quantity || it.quantity || 0,
                            ) || 0;
                          const unitPrice =
                            Number(
                              it.return_items_unit_price ||
                                it.sales_order_items_unit_price ||
                                0,
                            ) || 0;
                          const discountTotal =
                            Number(
                          ...
                          const itemDiscount =
                            qty > 0 ? (discountTotal / orderedQty) * qty : 0;
                          const tax = computeItemTax(it);
                          ...
                          return (
                            <tr key={idx} className="hover:bg-gray-50">
                              ...
                              <td className="px-3 py-2">
                                {it.packaging_type_name ||
                                  it.packaging_types_name ||
                                  "-"}
                              </td>
                              <td className="px-3 py-2">
                                {formatNumber(qty, { minFrac: 0, maxFrac: 2 })}
                              </td>
                              <td className="px-3 py-2">
