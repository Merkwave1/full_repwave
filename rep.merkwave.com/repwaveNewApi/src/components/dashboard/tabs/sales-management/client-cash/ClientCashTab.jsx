...
export default function ClientCashTab() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // api returns {movements, totals, pagination} — support both that and legacy array
      const movementList = Array.isArray(response)
        ? response
      ...
      const overall_total =
        Number(totals.overall_total ?? movementList.length) ||
        movementList.length;
      ...
      const nextTotalCount =
        Number(pagination.total_count ?? movementList.length) || 0;
      const nextTotalPages =
        Number(
          pagination.total_pages ??
            (nextTotalCount > 0 && perPage
              ? Math.ceil(nextTotalCount / perPage)
              : 1),
        ) || 1;
    ...
    const selector = (
      <div className="flex items-center gap-2" dir="rtl">
        ...
        <select
          ...
          className="px-3 py-2 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#8B5FD6] focus:border-[#8B5FD6]"
        >
  ...
  const columnsMemo = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <div className="flex items-center justify-center gap-2 text-sm font-semibold text-[#7A52C2]">
        ...
        render: (item) => (
          <div className="flex items-center justify-center gap-2">
            <button
              ...
              className="group p-1.5 text-[#8B5FD6] hover:text-white hover:bg-[#8B5FD6] rounded-full transition"
            >
    ...
    <>
      <CustomPageHeader
        ...
        actionButton={
          <button
            onClick={handleAdd}
            className="bg-white text-[#7A52C2] font-bold py-2 px-4 rounded-md shadow-md hover:bg-[#f5f3ff] flex items-center gap-2 transition-colors"
          >
