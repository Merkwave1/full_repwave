...
export default function TransfersTab() {
  ...
  const loadAllTransferData = useCallback(
    async (forceApiRefresh = false) => {
      ...
      try {
        ...
        setBaseUnits(Array.isArray(unitsData) ? unitsData : []);
        setPackagingTypes(Array.isArray(packagingTypesData) ? packagingTypesData : []);
  ...
  const openTransferDetailsModal = async (transferSummary) => {
    ...
    try {
      ...
      // Merge: keep the already-normalized summary (correct field names)
      // and attach the items array from the detailed API response.
      setSelectedTransfer({
        ...transferSummary,
  ...
  const handleApproveAllocateRequest = async (
    ...
    setLoading(true);
    try {
      // Build a transfer from selected allocations
      if (!Array.isArray(allocations) || allocations.length === 0) {
        throw new Error("برجاء اختيار الدُفعات والكميات قبل إنشاء التحويل.");
      }
      // Look up variant_id and packaging_type_id from allInventoryItems
  ...
  const filteredTransfers = useMemo(() => {
    ...
    // Normalize raw API transfers to the same field shape as mapped requests
    const mappedTransfers = (Array.isArray(transfers) ? transfers : []).map((t) => ({
      type: "transfer",
        ...
        return (
          <>
            <CustomPageHeader
              ...
              icon={<ArrowPathIcon className="h-8 w-8 text-[#1A0F35]" />}
