...
function TransferDetailsModal({
  ...
  useEffect(() => {
    if (transfer && Array.isArray(transfer.items)) {
      ...
      // Deep copy items; use transfer_item_id as fallback for inventory_id
      setEditableItems(transfer.items.map((item) => ({
        ...item,
        inventory_id: item.inventory_id ?? item.transfer_item_id,
      })));
      setItemErrors({});
                  ...
                  <div className="col-span-full">
                    ...
                    <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                      {groupedItem.packaging_details.map((pkg, pkgIndex) => {
                        ...
                        return (
                          <li
                            key={`pkg-${pkg.inventory_id ?? pkgIndex}`}
