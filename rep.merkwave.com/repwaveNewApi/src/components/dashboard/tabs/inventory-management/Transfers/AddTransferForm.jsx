...
export default function AddTransferForm({ onAdd, onCancel, warehouses, allInventoryItems, onRefreshSupporting, setGlobalMessage }) {
  ...
  const handleSubmit = (e) => {
    ...
    const flattenedItemsToSubmit = formData.items.flatMap(itemLine => itemLine.transfer_entries.map(entry => {
        const invItem = inventoryItemMap.get(String(entry.inventory_id));
        return {
            variant_id: invItem?.variant_id ?? parseInt(entry.inventory_id),
            packaging_type_id: invItem?.packaging_type_id ?? null,
            quantity: parseFloat(entry.quantity),
        };
    }));

    const payload = {
      from_warehouse_id: parseInt(formData.source_warehouse_id),
      to_warehouse_id: parseInt(formData.destination_warehouse_id),
          ...
          {formData.items.length === 0 && (
            <div className="text-center text-gray-600">
              ...
              <div className="mt-2 flex justify-center gap-3">
                <button type="button" onClick={handleAddItemLine} className="px-3 py-1.5 text-xs bg-[#8B5FD6] text-white rounded-md hover:bg-[#7A52C2]">إضافة أول عنصر</button>
              ...
              <div key={lineIndex} ref={lineIndex === formData.items.length - 1 ? lastItemRef : null} className="bg-gray-50 p-4 rounded-md shadow-sm mb-4 border border-gray-200 overflow-visible" style={{position: 'relative', zIndex: 'auto'}}>
                ...
                <div className="mb-4 overflow-visible">
                  ...
                  {formData.source_warehouse_id && availableVariantsForTransferOptions.length === 0 && (
                    <p className="mt-2 text-sm text-[#8B5FD6]">لا يوجد مخزون متاح في هذا المخزن لهذا التحويل حاليًا.</p>
                  )}
                </div>
