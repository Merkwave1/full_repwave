...
export default function LoadRequestsTab() {
  ...
  const unregisterTabRefreshHandler =
    outletContext?.unregisterTabRefreshHandler;
  ...
  const loadAllLoadRequestData = useCallback(
    async (forceApiRefresh = false) => {
      if (forceApiRefresh) {
        setLoading(true);
        setGlobalMessage?.({
          type: "info",
          message: "جاري تحديث طلبات التحميل...",
      ...
      try {
        ...
        const representativeCarWarehouses = warehousesList.filter(
          (w) =>
            w.warehouse_type === "Representative Car" ||
            w.warehouse_name.includes("سيارة المندوب"),
        );

        const filteredLoadRequests = (transfersData || [])
          .filter((transfer) =>
            representativeCarWarehouses.some(
              (repCarW) =>
                repCarW.warehouse_id === transfer.transfer_to_warehouse_id,
            ),
          )
          .map((t) => ({
            type: "transfer",
            transfer_id: t.transfer_id,
            display_id: t.transfer_id,
            transfer_source_warehouse_id: t.transfer_from_warehouse_id,
            transfer_destination_warehouse_id: t.transfer_to_warehouse_id,
            source_warehouse_name: t.from_warehouse_name,
            destination_warehouse_name: t.to_warehouse_name,
            status: t.transfer_status,
            created_at: t.transfer_date,
            notes: t.transfer_notes,
          }));
  ...
  useEffect(() => {
    ...
    if (registerTabRefreshHandler) {
      // فحص وجود الدالة قبل الاستدعاء
      registerTabRefreshHandler(tabName, loadAllLoadRequestData);
    }
    return () => {
      if (unregisterTabRefreshHandler) {
        // فحص وجود الدالة قبل الاستدعاء
        unregisterTabRefreshHandler(tabName);
      }
    };
  }, [
    loadAllLoadRequestData,
  ...
  const handleAddLoadRequest = async (newData) => {
    ...
    try {
      ...
      setGlobalMessage?.({
        type: "success",
        message: message || "تم إضافة طلب التحميل بنجاح!",
      }); // استخدام optional chaining
      setCurrentView("list");
      ...
      setGlobalMessage?.({
        type: "error",
        message: e.message || "فشل في إضافة طلب التحميل.",
      }); // استخدام optional chaining
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmStatusUpdate = async () => {
    ...
    try {
      ...
      setGlobalMessage?.({
        ...
        message:
          message ||
          `تم تحديث حالة طلب التحميل إلى ${newStatusForUpdate} بنجاح!`,
      }); // استخدام optional chaining
      ...
      setGlobalMessage?.({
        type: "error",
        message: e.message || "فشل في تحديث حالة طلب التحميل.",
      }); // استخدام optional chaining
    } finally {
      setLoading(false);
    }
  ...
  const filteredLoadRequests = useMemo(() => {
    ...
    if (term) {
      currentFiltered = currentFiltered.filter(
        (request) =>
          ...
          warehouses
            .find(
              (w) => w.warehouse_id === request.transfer_source_warehouse_id,
            )
          ...
          warehouses
            .find(
              (w) =>
                w.warehouse_id === request.transfer_destination_warehouse_id,
            )
            ?.warehouse_name.toLowerCase()
            .includes(term),
      );
    ...
    if (selectedSourceWarehouseFilter) {
      currentFiltered = currentFiltered.filter(
        (request) =>
          String(request.transfer_source_warehouse_id) ===
    ...
    if (selectedDestinationWarehouseFilter) {
      currentFiltered = currentFiltered.filter(
        (request) =>
          String(request.transfer_destination_warehouse_id) ===
          selectedDestinationWarehouseFilter,
      );
    }
          ...
          <>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
              ...
              <div className="relative flex-grow mx-4">
                <input
                  ...
                  className="w-full pl-4 pr-10 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-[#8B5FD6]"
                  dir="rtl"
                />
                {searchTerm && (
                  <button
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <label
                    htmlFor="destinationWarehouseFilter"
                    className="block text-sm font-medium text-gray-700 mb-1"
                  >
                    المخزن الوجهة (سيارة المندوب)
                  </label>
                  <SearchableSelect
                    options={warehouseOptions}
  ...
  return (
    <div className="p-4" dir="rtl">
      {renderContent()}
      {/* Add load request modal overlay */}
      {currentView === "add" && (
