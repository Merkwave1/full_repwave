...
const RecordsTabWrapper = () => {
  ...
  const loadAllData = useCallback(async () => {
    try {
      ...
      if (activeTab === "receiving") {
        try {
          const [purchaseOrdersRes, productsRes, suppliersRes] =
            await Promise.all([
              getAppPurchaseOrders(),
              getAppProducts(),
              getAppSuppliers(),
          ...
          setPurchaseOrders(
            (purchaseOrdersRes && (purchaseOrdersRes.data || [])) || [],
          );
  ...
  const getTabClasses = (isActive) =>
    `flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-t-lg transition-colors duration-200 ${
      isActive
        ? "bg-[#8B5FD6] text-white shadow-md"
        : "text-gray-700 hover:text-[#8B5FD6] hover:bg-gray-100 bg-white border-x border-t border-gray-200"
    }`;
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center gap-3">
        <DocumentTextIcon className="h-8 w-8 text-[#8B5FD6]" />
