        ...
        <div className="bg-white rounded-2xl shadow-md p-4 sm:p-6 border space-y-4">
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-4">
            ...
            <input
              type="number"
              min={0}
              max={parseFloat(inventoryItem?.inventory_quantity || 0)}
              step={conversionStep}
              value={quantityToConvert}
              onChange={(e) => {
                const rawVal = parseFloat(e.target.value) || 0;
                const max = parseFloat(inventoryItem?.inventory_quantity || 0);
                const step = selectedToPackagingType ? conversionStep || 1 : 1;
