...
import {
  ...
  updateInventory,
  repackInventory,
  markInventoryRemoved,
} from "../../../../../apis/inventory"; // Import repackInventory and markInventoryRemoved
...
function InventoryTab() {
  ...
  const [isInventoryDetailsModalOpen, setIsInventoryDetailsModalOpen] =
    useState(false);
  ...
  const loadSupportingData = useCallback(async (forceApiRefresh = false) => {
    try {
      ...
      const [productsData, warehousesData, unitsData, packagingTypesData] =
        await Promise.all([
      ...
      setError(
        "فشل في تحميل البيانات الأساسية: " + (err.message || "خطأ غير معروف"),
      );
    }
  }, []);
  ...
  const loadInventoryData = useCallback(async (force = false) => {
    ...
    try {
      ...
      setError(
        "فشل في تحميل بيانات المخزون: " + (err.message || "خطأ غير معروف"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const enrichedInventory = useMemo(() => {
    if (!Array.isArray(allInventoryItems) || !allInventoryItems.length) {
      return [];
    }

    return allInventoryItems.map((item) => {
      // Backend now returns product_name, variant_name, and products_id directly
      const variantDisplayName =
        item.variant_name || item.product_name || "Unknown Product/Variant";

      const foundWarehouse = Array.isArray(warehouses)
        ? warehouses.find((w) => w.warehouse_id === item.warehouse_id)
        : undefined;
      const warehouseName =
        foundWarehouse?.warehouse_name || "Unknown Warehouse";
      const warehouseCode = foundWarehouse?.warehouse_code || "";

      const packagingType = packagingTypes.find(
        (pt) =>
          pt.packaging_types_id === item.packaging_type_id ||
          pt.packaging_type_id === item.packaging_type_id,
      );

      // Resolve the product for unit_of_measure lookup
      const resolvedProduct =
        item.products_id && Array.isArray(products)
          ? products.find((p) => p.products_id === item.products_id)
          : null;

      return {
        ...item,
        products_name: item.product_name,
        ...
        products_unit_of_measure_id:
          resolvedProduct?.products_unit_of_measure_id ?? null,
      };
    });
  ...
  const filteredInventoryItems = useMemo(() => {
    ...
    if (selectedWarehouseFilter) {
      currentFiltered = currentFiltered.filter(
        (item) =>
          String(item.warehouse_id ?? "") === String(selectedWarehouseFilter),
      );
    }
    ...
    if (selectedVariantFilter) {
      currentFiltered = currentFiltered.filter(
        (item) => item.variant_display_name === selectedVariantFilter,
      );
    }
    ...
    if (selectedStatusFilter) {
      ...
      try {
        ...
        if (cached) {
          ...
          const out = inv.find(
            (s) => s.settings_key === "out_of_stock_threshold",
          );
          if (low && low.settings_value !== undefined)
            lowThreshold = parseFloat(low.settings_value);
          if (out && out.settings_value !== undefined)
            outThreshold = parseFloat(out.settings_value);
        }
      } catch {
        /* noop */
      }
      currentFiltered = currentFiltered.filter((item) => {
        ...
        const factor =
          parseFloat(packaging?.packaging_types_default_conversion_factor) || 1;
        ...
        else if (lowT !== undefined && lowT !== null && totalBase <= lowT)
          derived = "Low Stock";
        return (
          String(derived).toLowerCase() ===
          String(selectedStatusFilter).toLowerCase()
        );
      });
    }
  ...
  const totalVariantsCount = useMemo(() => {
    ...
    for (const item of filteredInventoryItems) {
      ...
      const key = item.variant_id
        ? `v:${item.variant_id}`
        : `vd:${item.variant_display_name || item.products_id}`;
      if (key) set.add(key);
    }
    return set.size;
  }, [filteredInventoryItems]);
  ...
  const handleAddInventory = async (newItemData) => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في إضافة عنصر المخزون.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateInventory = async (updatedItemData) => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في تحديث عنصر المخزون.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteInventory = async () => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: 'تم تحديث حالة عنصر المخزون إلى "Removed" بنجاح!',
      });
      setCurrentView("list");
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في حذف عنصر المخزون.",
      });
    } finally {
  ...
  const handleRepackConfirm = async (repackData) => {
    // NEW: Handler for confirming repack
    setLoading(true); // Show loader for repack operation
    try {
      const result = await repackInventory(repackData);
      const successMsg =
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في التحويل.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedWarehouseFilter) {
      const option = warehouseOptions.find(
        (o) => o.value === selectedWarehouseFilter,
      );
      if (option)
        chips.push({
          key: "warehouse",
          label: "المخزن",
          value: option.label,
          onRemove: () => setSelectedWarehouseFilter(""),
        });
    }
    if (selectedVariantFilter) {
      const option = variantOptions.find(
        (o) => o.value === selectedVariantFilter,
      );
      if (option)
        chips.push({
          key: "variant",
          label: "الخيار",
          value: option.label,
          onRemove: () => setSelectedVariantFilter(""),
        });
    }
    if (selectedStatusFilter) {
  ...
  const renderContent = () => {
    ...
    const isSupportingDataReady =
      products.length > 0 && warehouses.length > 0 && packagingTypes.length > 0;
        ...
        return (
          <>
            <CustomPageHeader
              ...
              icon={<CubeIcon className="h-8 w-8 text-[#1A0F35]" />}
            ...
            <FilterBar
              searchConfig={{
                ...
                placeholder: "ابحث عن عنصر مخزون...",
              ...
              <ProductInventorySummary
                ...
                onShowDetails={(payload) => {
                  ...
                  if (payload?.variantId) {
                    ...
                    const augmented = {
                      ...(payload.product || {}),
                      __selectedVariantId: payload.variantId,
                    };
  ...
  return (
    <div className="p-4" dir="rtl">
      ...
      {/* Edit inventory modal overlay */}
      {currentView === "edit" && selectedInventory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black/40 backdrop-blur-sm p-4">
          <UpdateInventoryForm
        ...
        <ProductInventoryDetailsModal
          ...
          filters={{
            warehouse: selectedWarehouseFilter
              ? warehouses.find(
                  (w) =>
                    String(w.warehouse_id) === String(selectedWarehouseFilter),
                )?.warehouse_name
              : null,
