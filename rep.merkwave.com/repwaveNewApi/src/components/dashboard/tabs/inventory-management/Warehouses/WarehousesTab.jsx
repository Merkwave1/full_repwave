...
function WarehousesTab() {
  ...
  const handleUpdateWarehouse = async (updatedWarehouseData) => {
    try {
      ...
      const message =
        typeof result === "string" ? result : "تم تحديث المخزن بنجاح!";
      setGlobalMessage({
        type: "success",
        message,
      });
        ...
        return (
          <>
            <CustomPageHeader
              ...
              icon={
                <BuildingStorefrontIcon className="h-8 w-8 text-[#1A0F35]" />
              }
                ...
                columns={[
                  {
                    ...
                    sortable: true,
                    render: (row) => (
                      <span className="text-[#C4A8F0] bg-[#1A0F35] text-xs px-2 py-1 rounded-full font-semibold">
                        {row.warehouse_id}
                      </span>
                    ),
                  },
