  ...
  const columns = [
    {
      ...
      render: (p) => (
        <span className="bg-[#f5f3ff] text-[#1A0F35] text-xs px-2 py-1 rounded-full font-semibold">
          {p.products_id}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (p) => (
        <span
          className={`font-semibold text-xs px-2 py-1 rounded-full ${p.products_is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
          {p.products_is_active ? "نشط" : "غير نشط"}
        </span>
      ),
    },
    {
