...
import {
  getAllProductAttributesWithValues,
  addProductAttributeWithValues,
  updateProductAttributeWithValues,
  deleteProductAttribute,
} from "../../../../apis/product_attributes";
...
export default function AttributesTab() {
  ...
  const handleAddAttribute = async (formData) => {
    setLoading(true);
    try {
      await addProductAttributeWithValues(
  ...
  const handleUpdateAttribute = async (formData) => {
    setLoading(true);
    try {
      await updateProductAttributeWithValues(
  ...
  switch (view) {
    case "delete":
    default: // 'list', 'add', 'edit', 'details' all show the list
      content = (
        <>
          <CustomPageHeader
            title="إدارة الخصائص"
            ...
            actionButton={
              <button
                onClick={() => setView("add")}
                className="bg-white text-[#8B5FD6] font-bold py-2 px-4 rounded-md shadow-md"
              >
                إضافة خاصية
              </button>
            }
