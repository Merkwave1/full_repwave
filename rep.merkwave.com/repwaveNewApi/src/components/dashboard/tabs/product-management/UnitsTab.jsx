...
export default function UnitsTab() {
  ...
  const handleUpdateUnit = async (formData) => {
    setLoading(true);
    try {
      await updateBaseUnit(selectedUnit.base_units_id, formData);
      setGlobalMessage({
        type: "success",
    ...
    <div className="p-4" dir="rtl">
      ...
      {view === "add" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black/40 backdrop-blur-sm p-4">
          <AddUnitForm onAdd={handleAddUnit} onCancel={() => setView("list")} />
        </div>
      )}
